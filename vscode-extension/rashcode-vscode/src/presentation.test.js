...
test('truncateMiddle keeps the profile filename visible', () => {
  ...
  assert.equal(
    truncateMiddle('/Users/example/projects/rashcode/workspace/.rashcode-profile.json', 30),
    '.../.rashcode-profile.json',
  );
});
...
test('truncateMiddle keeps the filename visible for Windows-style paths', () => {
  ...
  assert.equal(
    truncateMiddle('C:\\Users\\example\\rashcode\\workspace\\.rashcode-profile.json', 30),
    '...\\.rashcode-profile.json',
  );
});
...
test('buildActionModel includes workspace-profile action when a profile exists', () => {
  ...
  const model = buildActionModel({
    canLaunchInWorkspaceRoot: true,
    workspaceProfilePath: 'C:\\Users\\example\\rashcode\\workspace\\.rashcode-profile.json',
  });

  assert.deepEqual(model.openProfile, {
    id: 'openProfile',
    label: 'Open Workspace Profile',
    detail: 'Inspect ...\\.rashcode-profile.json',
    tone: 'neutral',
    disabled: false,
  });
});
...
function createStatus(overrides = {}) {
  return {
    ...
    shimEnabled: false,
    workspaceFolder: '/workspace/rashcode',
    ...
    profileStatusHint: '/workspace/rashcode/.rashcode-profile.json',
    workspaceProfilePath: '/workspace/rashcode/.rashcode-profile.json',
    providerState: {
      label: 'Codex',
      detail: 'gpt-5.4',
      source: 'profile',
      ...
      rows: [
        {
          ...
          summary: 'rashcode',
          detail: '/workspace/rashcode · active editor workspace',
        },
        {
          key: 'profileStatus',
          label: 'Workspace profile',
          summary: 'Found',
          detail: '/workspace/rashcode/.rashcode-profile.json',
          tone: 'neutral',
        },
      ],
    },
...
test('buildControlCenterViewModel keeps launch command only in summary cards', () => {
  ...
  assert.deepEqual(viewModel.summaryCards.find(card => card.key === 'launchCommand'), {
    ...
    value: 'rashcode --project-aware',
    detail: 'Integrated terminal: rashcode',
  });
