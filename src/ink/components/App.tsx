...
export default class App extends PureComponent<Props, State> {
  ...
  stdinMode: 'readable' | 'data' = process.env.rashcode_USE_READABLE_STDIN === '1' ? 'readable' : 'data';
  ...
  override componentDidMount() {
    ...
    if (this.props.stdout.isTTY && !isEnvTruthy(process.env.RASH_CODE_ACCESSIBILITY)) {
  ...
  handleSuspend = (): void => {
    ...
    // Emit suspend event for Rash Code to handle. Mostly just has a notification
    ...
    const resumeHandler = () => {
      ...
      if (this.props.stdout.isTTY) {
        if (!isEnvTruthy(process.env.RASH_CODE_ACCESSIBILITY)) {
          this.props.stdout.write(HIDE_CURSOR);
        }
      ...
      // Emit resume event for Rash Code to handle
