// OAuth client for handling authentication flows with Rash services
...
/**
 * Check if the user has Rash.ai authentication scope
 * @private Only call this if you're OAuth / auth related code!
 */
export function shouldUseRashAIAuth(scopes: string[] | undefined): boolean {
  return Boolean(scopes?.includes(RASH_AI_INFERENCE_SCOPE))
}
...
export function buildAuthUrl({
  ...
  port: number
  isManual: boolean
  loginWithRashAi?: boolean
  inferenceOnly?: boolean
  orgUUID?: string
  loginHint?: string
  loginMethod?: string
}): string {
  const authUrlBase = loginWithRashAi
    ? getOauthConfig().RASH_AI_AUTHORIZE_URL
    : getOauthConfig().CONSOLE_AUTHORIZE_URL

  const authUrl = new URL(authUrlBase)
  authUrl.searchParams.append('code', 'true') // this tells the login page to show Rash Max upsell
  ...
  const scopesToUse = inferenceOnly
    ? [RASH_AI_INFERENCE_SCOPE] // Long-lived inference-only tokens
    : ALL_OAUTH_SCOPES
...
export async function refreshOAuthToken(
  ...
  const requestBody = {
    ...
    client_id: getOauthConfig().CLIENT_ID,
    // Request specific scopes, defaulting to the full Rash AI set. The
  ...
  try {
    ...
    // RASH_CODE_OAUTH_REFRESH_TOKEN re-login path: installOAuthTokens runs
    ...
    const existing = getRashAIOAuthTokens()
...
export async function populateOAuthAccountInfoIfNeeded(): Promise<boolean> {
  ...
  const envAccountUuid = process.env.RASH_CODE_ACCOUNT_UUID
  const envUserEmail = process.env.RASH_CODE_USER_EMAIL
  const envOrganizationUuid = process.env.RASH_CODE_ORGANIZATION_UUID
