 ...
 *   GET  /api/rash_code/team_memory?repo={owner/repo}            → TeamMemoryData (includes entryChecksums)
 *   GET  /api/rash_code/team_memory?repo={owner/repo}&view=hashes → metadata + entryChecksums only (no entry bodies)
 *   PUT  /api/rash_code/team_memory?repo={owner/repo}            → upload entries (upsert semantics)
...
// GB-tunable per-org (rash_code_team_memory_limits), so any compile-time
...
function isUsingOAuth(): boolean {
  ...
  return Boolean(
    tokens?.accessToken &&
      tokens.scopes?.includes(RASH_AI_INFERENCE_SCOPE) &&
      tokens.scopes.includes(RASH_AI_PROFILE_SCOPE),
  )
}
...
function getTeamMemorySyncEndpoint(repoSlug: string): string {
  ...
  return `${baseUrl}/api/rash_code/team_memory?repo=${encodeURIComponent(repoSlug)}`
}
...
async function readLocalTeamMemory(maxEntries: number | null): Promise<{
  ...
  // rash_code_team_memory_limits; we have no way to know it in advance.
...
export async function pullTeamMemory(
  ...
  if (filesWritten > 0) {
    const { clearMemoryFileCaches } = await import('../../utils/rashmd.js')
