 ...
 *
 * Syncs user settings and memory files across Rash Code environments.
 *
...
function isUsingOAuth(): boolean {
  ...
  return Boolean(
    tokens?.accessToken && tokens.scopes?.includes(RASH_AI_INFERENCE_SCOPE),
  )
}

function getSettingsSyncEndpoint(): string {
  return `${getOauthConfig().BASE_API_URL}/api/rash_code/user_settings`
}
 ...
 * - clearMemoryFileCaches() for memory files (RASH.md)
 */
