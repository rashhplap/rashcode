...
import { RashError } from '../utils/errors.js'
import { normalizePathForComparison, pathsEqual } from '../utils/file.js'
import { getConnectedIdeClient } from '../utils/ide.js'
import { jsonParse } from '../utils/slowOperations.js'

class DiagnosticsTrackingError extends RashError {}
...
export class DiagnosticTrackingService {
  ...
  // Map<normalizedPath, lastRashFsRightDiagnostics>
  ...
  /**
   * Get new diagnostics from file://, _rash_fs_right, and _rash_fs_ URIs that aren't in the baseline.
  ...
  async getNewDiagnostics(): Promise<DiagnosticFile[]> {
    ...
    const diagnosticsForRashFsRightUrisWithBaselinesMap = new Map<
      string,
      DiagnosticFile
    >()
    allDiagnosticFiles
      .filter(file => this.baseline.has(this.normalizeFileUri(file.uri)))
      .filter(file => file.uri.startsWith('_rash_fs_right:'))
      .forEach(file => {
        diagnosticsForRashFsRightUrisWithBaselinesMap.set(
    ...
    for (const file of diagnosticsForFileUrisWithBaselines) {
      ...
      // Get the _rash_fs_right file if it exists
      const rashFsRightFile =
        diagnosticsForRashFsRightUrisWithBaselinesMap.get(normalizedPath)
      ...
      if (rashFsRightFile) {
        ...
        if (
          ...
          !this.areDiagnosticArraysEqual(
            previousRightDiagnostics,
            rashFsRightFile.diagnostics,
          )
        ) {
          fileToUse = rashFsRightFile
        }
