...
// Skills can be large (verify=18.7KB, rash-api=20.1KB). Previously re-injected
  ...
  try {
    if (promptCacheSharingEnabled) {
      try {
        ...
        // budget_tokens via Math.min(budget, maxOutputTokens-1) in rash.ts,
...
function shouldExcludeFromPostCompactRestore(
  ...
  // Exclude all types of rash.md files
  // TODO: Refactor to use isMemoryFilePath() from rashmd.ts for consistency
  // and to also match child directory memory files (.rash/rules/*.md, etc.)
  try {
