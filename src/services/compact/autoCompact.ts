...
import { getMaxOutputTokensForModel } from '../api/rash.js'
...
export function getEffectiveContextWindowSize(model: string): number {
  ...
  const autoCompactWindow = process.env.RASH_CODE_AUTO_COMPACT_WINDOW
...
export function getAutoCompactThreshold(model: string): number {
  ...
  const envPercent = process.env.RASH_AUTOCOMPACT_PCT_OVERRIDE
  if (envPercent) {
...
export function calculateTokenWarningState(
  ...
  const blockingLimitOverride = process.env.RASH_CODE_BLOCKING_LIMIT_OVERRIDE
...
export async function shouldAutoCompact(
  ...
  // RASH_CONTEXT_COLLAPSE env override is honored here too. require()
