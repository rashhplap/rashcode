 ...
 * - OAuth users (Rash.ai): Only Team and Enterprise/C4E subscribers are eligible
...
function getCachePath(): string {
  return join(getRashConfigHomeDir(), CACHE_FILENAME)
}
...
function getPolicyLimitsEndpoint(): string {
  return `${getOauthConfig().BASE_API_URL}/api/rash_code/policy_limits`
}
...
export function isPolicyLimitsEligible(): boolean {
  ...
  // For OAuth users, check if they have Rash.ai tokens
  const tokens = getRashAIOAuthTokens()
  if (!tokens?.accessToken) {
    return false
  }

  // Must have Rash.ai inference scope
  if (!tokens.scopes?.includes(RASH_AI_INFERENCE_SCOPE)) {
    return false
  }
...
function getAuthHeaders(): {
  ...
  // Fall back to OAuth tokens (for Rash.ai users)
