 ...
 * reply and emits notifications/rash/channel/permission with
 ...
 * capabilities.experimental['rash/channel/permission'].
 *
 * Kenneth's "would this let Rash self-approve?": the approving party is
 * the human via the channel, not Rash. But the trust boundary isn't the
...
export type ChannelPermissionCallbacks = {
   ...
   *  (notifications/rash/channel/permission). Returns true if the ID
 ...
 * reply and emits notifications/rash/channel/permission with {request_id,
...
>(
  ...
  return clients.filter(
    (c): c is T & { type: 'connected' } =>
      ...
      c.capabilities?.experimental?.['rash/channel'] !== undefined &&
      c.capabilities?.experimental?.['rash/channel/permission'] !== undefined,
  )
}

/**
 * Factory for the callbacks object. The pending Map is closed over — NOT
 * module-level (per src/RASH.md), NOT in AppState (functions-in-state
 ...
 * (notifications/rash/channel/permission) with the structured payload.
