 ...
 * notify VSCode when files are edited or written by Rash.
 */
...
export function setupVscodeSdkMcp(sdkClients: MCPServerConnection[]): void {
  const client = sdkClients.find(client => client.name === 'rash-vscode')
  ...
  if (client && client.type === 'connected') {
    ...
    const gates: Record<string, boolean | string> = {
      ...
      // In-band OAuth via rash_authenticate (vs. extension-native PKCE).
