 ...
 * Used by RashAuthProvider for metadata discovery and token refresh.
 ...
 *
 * Per RFC 7009, public clients (like Rash Code) should authenticate by including
 ...
 * `rash mcp xaa setup`). Per-server config is just `oauth.xaa: true`
...
async function performMCPXaaAuth(
  ...
  if (!idp) {
    throw new Error(
      "XAA: no IdP connection configured. Run 'rash mcp xaa setup --issuer <url> --client-id <id> --client-secret' to configure.",
    )
  }
  ...
  try {
    ...
    // (instead of RashAuthProvider.saveTokens) to avoid instantiating the
...
export async function performMCPOAuthFlow(
  ...
  // rest of CC's transport wiring (RashAuthProvider.tokens() in client.ts)
  // works unchanged.
  //
  ...
  //
  // Servers with `oauth.xaa` but RASH_CODE_ENABLE_XAA unset hard-fail with
  // actionable copy rather than silently degrade to consent.
  if (serverConfig.oauth?.xaa) {
    if (!isXaaEnabled()) {
      throw new Error(
        `XAA is not enabled (set RASH_CODE_ENABLE_XAA=1). Remove 'oauth.xaa' from server '${serverName}' to use the standard consent flow.`,
      )
    }
  ...
  try {
    ...
    const provider = new RashAuthProvider(
      serverName,
      serverConfig,
      redirectUri,
      true,
      ...
      server = createServer((req, res) => {
        ...
        if (parsedUrl.pathname === '/callback') {
          ...
          if (code) {
            res.writeHead(200, { 'Content-Type': 'text/html' })
            res.end(
              `<h1>Authentication Successful</h1><p>You can close this window. Return to Rash Code.</p>`,
            )
            cleanup()
            resolveOnce(code)
          }
 ...
 * to the PKCE flow. See github.com/anthropics/rash-code/issues/28258.
 */
export function wrapFetchWithStepUpDetection(
  baseFetch: FetchLike,
  provider: RashAuthProvider,
): FetchLike {
...
export class RashAuthProvider implements OAuthClientProvider {
  ...
  get clientMetadata(): OAuthClientMetadata {
    const metadata: OAuthClientMetadata = {
      client_name: `Rash Code (${this.serverName})`,
   ...
   * but this is the shape RASH.md flags under "Token/auth caching across
  ...
  async refreshAuthorization(
    ...
    const lockfilePath = join(rashDir, `mcp-refresh-${sanitizedKey}.lock`)
