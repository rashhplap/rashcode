 ...
 *   - sends `notifications/rash/channel` notifications for inbound — this file
 *
 ...
 * Requires rash.ai OAuth auth — API key users are blocked until
...
export const ChannelMessageNotificationSchema = lazySchema(() =>
  z.object({
    method: z.literal('notifications/rash/channel'),
    params: z.object({
      content: z.string(),
 ...
 * this declare `capabilities.experimental['rash/channel/permission']` and
 * emit this event INSTEAD of relaying "yes tbxkq" as text via
 * notifications/rash/channel. Explicit opt-in per server — a channel that
...
export const CHANNEL_PERMISSION_METHOD =
  'notifications/rash/channel/permission'
...
export const CHANNEL_PERMISSION_REQUEST_METHOD =
  'notifications/rash/channel/permission_request'
...
export function gateChannelServer(
  ...
  // Channel servers declare `experimental['rash/channel']: {}` (MCP's
  // presence-signal idiom — same as `tools: {}`). Truthy covers `{}` and
  // `true`; absent/undefined/explicit-`false` all fail. Key matches the
  // notification method namespace (notifications/rash/channel).
  if (!capabilities?.experimental?.['rash/channel']) {
    return {
      action: 'skip',
      kind: 'capability',
      reason: 'server did not declare rash/channel capability',
    }
  }
  ...
  if (!getRashAIOAuthTokens()?.accessToken) {
    return {
      action: 'skip',
      kind: 'auth',
      reason: 'channels requires rash.ai authentication (run /login)',
    }
  }
