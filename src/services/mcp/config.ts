...
import { isRashInChromeMCPServer } from '../../utils/rashInChrome/common.js'
import {
...
import { fetchRashAIMcpConfigsIfEligible } from './rashai.js'
...
/**
 * CCR proxy URL path markers. In remote sessions, rash.ai connectors arrive
 ...
 * plugin deduplication. Ignores env (plugins always inject RASH_PLUGIN_ROOT)
...
/**
 * Filter rash.ai connectors, dropping any whose signature matches an enabled
 * manually-configured server. Manual wins: a user who wrote .mcp.json or ran
 * `rash mcp add` expressed higher intent than a connector toggled in the web UI.
 *
 * Connector keys are `rash.ai <DisplayName>` so they never key-collide with
 ...
 * `mcp__rash_ai_Slack__*` both hitting mcp.slack.com, ~600 chars/turn wasted).
 *
 ...
 */
export function dedupRashAiMcpServers(
  rashAiServers: Record<string, ScopedMcpServerConfig>,
  manualServers: Record<string, ScopedMcpServerConfig>,
): {
  servers: Record<string, ScopedMcpServerConfig>
  suppressed: Array<{ name: string; duplicateOf: string }>
  ...
  const servers: Record<string, ScopedMcpServerConfig> = {}
  const suppressed: Array<{ name: string; duplicateOf: string }> = []
  for (const [name, config] of Object.entries(rashAiServers)) {
    const sig = getMcpServerSignature(config)
    const manualDup = sig !== null ? manualSigs.get(sig) : undefined
    if (manualDup !== undefined) {
      logForDebugging(
        `Suppressing rash.ai connector "${name}": duplicates manually-configured "${manualDup}"`,
      )
      suppressed.push({ name, duplicateOf: manualDup })
      continue
    }
 ...
 * in getRashCodeMcpConfigs(): --mcp-config (main.tsx) and the mcp_set_servers
...
export async function addMcpConfig(
  ...
  // Block reserved server name "rash-in-chrome"
  if (isRashInChromeMCPServer(name)) {
  ...
  switch (scope) {
    ...
    case 'rashai':
      throw new Error('Cannot add MCP server to scope: rashai')
  }
...
export function getMcpConfigByName(name: string): ScopedMcpServerConfig | null {
  ...
  // by name. User/project/local servers are blocked — same as getRashCodeMcpConfigs().
...
/**
 * Get Rash Code MCP configurations (excludes rash.ai servers from the
 ...
 * rash.ai connector fetch) is awaited only after loadAllPluginsCacheOnly() completes,
 * so the two overlap rather than serialize.
 * @returns Rash Code server configurations with appropriate scopes
 */
export async function getRashCodeMcpConfigs(
...
/**
 * Get all MCP configurations across all scopes, including rash.ai servers.
 * This may be slow due to network calls - use getRashCodeMcpConfigs() for fast startup.
...
export async function getAllMcpConfigs(): Promise<{
  ...
  // In enterprise mode, don't load rash.ai servers (enterprise has exclusive control)
  if (doesEnterpriseMcpConfigExist()) {
    return getRashCodeMcpConfigs()
  }

  // Kick off the rash.ai fetch before getRashCodeMcpConfigs so it overlaps
  // with loadAllPluginsCacheOnly() inside. Memoized — the awaited call below is a cache hit.
  const rashaiPromise = fetchRashAIMcpConfigsIfEligible()
  const { servers: rashCodeServers, errors } = await getRashCodeMcpConfigs(
    {},
    rashaiPromise,
  )
  const { allowed: rashaiMcpServers } = filterMcpServersByPolicy(
    await rashaiPromise,
  )

  // Suppress rash.ai connectors that duplicate an enabled manual server.
  // Keys never collide (`slack` vs `rash.ai Slack`) so the merge below
  // won't catch this — need content-based dedup by URL signature.
  const { servers: dedupedRashAi } = dedupRashAiMcpServers(
    rashaiMcpServers,
    rashCodeServers,
  )

  // Merge with rash.ai having lowest precedence
  const servers = Object.assign({}, dedupedRashAi, rashCodeServers)
  ...
  for (const [name, config] of Object.entries(schemaResult.data.mcpServers)) {
    ...
    if (
      ...
      errors.push({
        ...
        suggestion: `Change command to "cmd" with args ["/c", "npx", ...]. See: https://code.rash.com/docs/en/mcp#configure-mcp-servers`,
        mcpErrorMetadata: {
          scope,
          serverName: name,
          severity: 'warning',
...
export function areMcpConfigsAllowedWithEnterpriseMcpConfig(
  ...
  // what the best way to do this is. In the meantime, we are limiting this to rash-vscode for now to
  ...
  return Object.values(configs).every(
    c => c.type === 'sdk' && c.name === 'rash-vscode',
  )
}
