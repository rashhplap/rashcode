...
import { getGlobalRashFile } from '../../utils/env.js'
...
export function describeMcpConfigFilePath(scope: ConfigScope): string {
  switch (scope) {
    case 'user':
      return getGlobalRashFile()
    case 'project':
      return join(getCwd(), '.mcp.json')
    case 'local':
      return `${getGlobalRashFile()} [project: ${getCwd()}]`
    case 'dynamic':
...
export function getProjectMcpServerStatus(
  ...
  // flagSettings/policySettings but NOT projectSettings (repo-level .rash/settings.json).
  ...
  // In non-interactive mode (SDK, rash -p, piped input), there's no way to
...
export function getMcpServerScopeFromToolName(
  ...
  // Fallback: rash.ai servers have normalized names starting with "rash_ai_"
  // but aren't in getMcpConfigByName (they're fetched async separately)
  if (!serverConfig && mcpInfo.serverName.startsWith('rash_ai_')) {
    return 'rashai'
  }
...
export function extractAgentMcpServers(
  ...
  for (const [name, { config, sourceAgents }] of serverMap) {
    ...
    // Skip unsupported transport types (sdk, rashai-proxy, sse-ide, ws-ide)
