...
import { getRashTempDir } from 'src/utils/permissions/filesystem.js'
...
function getCacheBreakDiffPath(): string {
  ...
  return join(getRashTempDir(), `cache-break-${suffix}.diff`)
}
...
type PreviousState = {
   ...
   *  (sticky-on latched in rash.ts). Tracked to verify the fix. */
  ...
  effortValue: string
  /** Hash of getExtraBodyParams() — catches RASH_CODE_EXTRA_BODY and
...
async function writeCacheBreakDiff(
  ...
  try {
    ...
    await mkdir(getRashTempDir(), { recursive: true })
