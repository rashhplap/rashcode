...
import { hasProfileScope, isRashAISubscriber } from '../../utils/auth.js'
...
// N `rash -p` invocations into ~1 API call/day.
...
async function _fetchMetricsEnabled(): Promise<MetricsEnabledResponse> {
  ...
  const endpoint = `https://api.anthropic.com/api/rash_code/organizations/metrics_enabled`
...
export async function checkMetricsEnabled(): Promise<MetricsStatus> {
  ...
  if (isRashAISubscriber() && !hasProfileScope()) {
    return { enabled: false, hasError: false }
  }
