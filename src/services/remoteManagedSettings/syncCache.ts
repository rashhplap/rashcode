...
import { RASH_AI_INFERENCE_SCOPE } from '../../constants/oauth.js'
import {
 ...
 *   RASH_CODE_OAUTH_TOKEN / FD, or keychain tokens missing metadata): Eligible —
...
export function isRemoteManagedSettingsEligible(): boolean {
  ...
  // Check OAuth first: most Rash.ai users have no API key in the keychain.
  ...
  // Externally-injected tokens (CCD via RASH_CODE_OAUTH_TOKEN, CCR via
  // RASH_CODE_OAUTH_TOKEN_FILE_DESCRIPTOR, Agent SDK, CI) carry no
  // subscriptionType metadata — getRashAIOAuthTokens() constructs them with
