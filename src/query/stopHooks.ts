...
> {
  ...
  // the turn returns — otherwise `rash list` shows stale state for the gap.
  ...
  if (
    ...
    const p = jobClassifierModule!
      .classifyAndWriteState(process.env.RASH_JOB_DIR, turnAssistantMessages)
      .catch(err => {
  ...
  if (!isBareMode()) {
    ...
    if (!isEnvDefinedFalsy(process.env.RASH_CODE_ENABLE_PROMPT_SUGGESTION)) {
