...
/**
 * Rash Code parses history for pasted content references to match back to
...
async function* makeLogEntryReader(): AsyncGenerator<LogEntry> {
  ...
  const historyPath = join(getRashConfigHomeDir(), 'history.jsonl')
...
export function addToHistory(command: HistoryEntry | string): void {
  // Skip history when running in a tmux session spawned by Rash Code's Tungsten tool.
  // This prevents verification/test sessions from polluting the user's real command history.
  if (isEnvTruthy(process.env.RASH_CODE_SKIP_PROMPT_HISTORY)) {
    return
  }
