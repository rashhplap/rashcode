...
export function useUnseenDivider(messageCount: number): {
  ...
  const onScrollAway = useCallback((handle: ScrollBoxHandle) => {
    ...
    //   at max (Sarah Deaton, #rash-code-feedback 2026-03-15)
 ...
 * users think of as "a new message from Rash" — not raw assistant entries
 ...
 * until Rash's text response lands.
 */
 ...
 *
 * Fullscreen mode defaults on for ants (RASH_CODE_NO_FLICKER=0 to opt out)
 * and off for external users (RASH_CODE_NO_FLICKER=1 to opt in).
...
// conversation turn's prompt above the viewport so you know what Rash was
