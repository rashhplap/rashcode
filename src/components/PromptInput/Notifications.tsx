...
import { useRashAiLimits } from '../../services/rashAiLimitsHook.js';
...
export function Notifications(t0) {
  ...
  const rashAiLimits = useRashAiLimits();
  let t5;
  let t6;
  ...
  const isInOverageMode = rashAiLimits.isUsingOverage;
  let t7;
  ...
  return <>
      ...
      {(apiKeyStatus === 'invalid' || apiKeyStatus === 'missing') && <Box>
          <Text color="error" wrap="truncate">
            {isEnvTruthy(process.env.RASH_CODE_REMOTE) ? 'Authentication error · Try again' : 'Not logged in · Run /login'}
          </Text>
        </Box>}
      {debug && <Box>
          <Text color="warning" wrap="truncate">
