  ...
  useEffect(() => {
    if (feature('ULTRAPLAN') && ultraplanTriggers.length) {
      addNotification({
        ...
        text: 'This prompt will launch an ultraplan session in Rash Code on the web',
        priority: 'immediate',
        timeoutMs: 5000
      });
    } else {
  ...
  useEffect(() => {
    if (isUltrareviewEnabled() && ultrareviewTriggers.length) {
      addNotification({
        ...
        text: 'Run /ultrareview after Rash finishes to review these changes in the cloud',
        priority: 'immediate',
        timeoutMs: 5000
      });
    }
