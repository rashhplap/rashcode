...
export type PermissionHandlerOptions = {
  ...
  scope?: 'rash-folder' | 'global-rash-folder'
}
...
function handleAcceptSession(
  ...
  // For rash-folder scope, grant session-level access to all .rash/ files
  if (
    options?.scope === 'rash-folder' ||
    options?.scope === 'global-rash-folder'
  ) {
    const pattern =
      options.scope === 'global-rash-folder'
        ? GLOBAL_RASH_FOLDER_PERMISSION_PATTERN
