...
export function getDownloadUrl(): string {
  switch (process.platform) {
    case 'win32':
      return 'https://rash.ai/api/desktop/win32/x64/exe/latest/redirect';
    default:
      return 'https://rash.ai/api/desktop/darwin/universal/dmg/latest/redirect';
  }
}
...
export function DesktopHandoff(t0) {
    ...
    t2 = () => {
      const performHandoff = async function performHandoff() {
        ...
        if (installStatus.status === "not-installed") {
          setDownloadMessage("Rash Desktop is not installed.");
          setState("prompt-download");
          return;
        }
        if (installStatus.status === "version-too-old") {
          setDownloadMessage(`Rash Desktop needs to be updated (found v${installStatus.version}, need v1.1.2396+).`);
          setState("prompt-download");
          return;
        }
        setState("flushing");
  ...
  if ($[17] === Symbol.for("react.memo_cache_sentinel")) {
    t4 = {
      checking: "Checking for Rash Desktop\u2026",
      flushing: "Saving session\u2026",
      opening: "Opening Rash Desktop\u2026",
      success: "Opening in Rash Desktop\u2026"
    };
    $[17] = t4;
  } else {
    t4 = $[17];
...
async function _temp2(onDone_0) {
  onDone_0("Session transferred to Rash Desktop", {
    display: "system"
  });
