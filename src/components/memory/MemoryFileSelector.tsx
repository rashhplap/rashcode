...
import { getMemoryFiles, type MemoryFileInfo } from '../../utils/rashmd.js';
...
export function MemoryFileSelector(t0) {
  ...
  const userMemoryPath = join(getRashConfigHomeDir(), "RASH.md");
  const projectMemoryPath = join(getOriginalCwd(), "RASH.md");
  ...
  const memoryOptions = allMemoryFiles.map(file => {
    ...
    if (file.type === "User" && !file.isNested) {
      description = "Saved in ~/.rash/RASH.md";
    } else {
      if (file.type === "Project" && !file.isNested && file.path === projectMemoryPath) {
        description = `${isGit ? "Checked in at" : "Saved in"} ./RASH.md`;
      } else {
        if (file.parent) {
