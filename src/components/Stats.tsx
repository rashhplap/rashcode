...
import { aggregateRashCodeStatsForRange, type RashCodeStats, type DailyModelTokens, type StatsDateRange } from '../utils/stats.js';
...
function createAllTimeStatsPromise(): Promise<StatsResult> {
  return aggregateRashCodeStatsForRange('all').then((data): StatsResult => {
...
export function Stats(t0) {
  ...
  if ($[1] === Symbol.for("react.memo_cache_sentinel")) {
    t2 = <Box marginTop={1}><Spinner /><Text> Loading your Rash Code stats…</Text></Box>;
    $[1] = t2;
  } else {
    t2 = $[1];
  }
...
function StatsContent(t0) {
  ...
  if ($[1] !== dateRange || $[2] !== statsCache) {
    t2 = () => {
      ...
      aggregateRashCodeStatsForRange(dateRange).then(data => {
        if (!cancelled) {
          setStatsCache(prev => ({
            ...prev,
            [dateRange]: data
  ...
  if (allTimeResult.type === "empty") {
    ...
    if ($[15] === Symbol.for("react.memo_cache_sentinel")) {
      t7 = <Box marginTop={1}><Text color="warning">No stats available yet. Start using Rash Code!</Text></Box>;
      $[15] = t7;
    } else {
      t7 = $[15];
    }
  ...
  let t9;
  if ($[26] !== t7 || $[27] !== t8) {
    t9 = <Box flexDirection="row" gap={1} marginBottom={1}><Tabs title="" color="rash" defaultTab="Overview">{t7}{t8}</Tabs></Box>;
    $[26] = t7;
    $[27] = t8;
    $[28] = t9;
  } else {
  ...
  let t12;
  if ($[31] !== t11 || $[32] !== t9) {
    t12 = <Pane color="rash">{t9}{t11}</Pane>;
    $[31] = t11;
    $[32] = t9;
    $[33] = t12;
  } else {
...
function DateRangeSelector(t0) {
  ...
  let t1;
  if ($[0] !== dateRange) {
    t1 = DATE_RANGE_ORDER.map((range, i) => <Text key={range}>{i > 0 && <Text dimColor={true}> · </Text>}{range === dateRange ? <Text bold={true} color="rash">{DATE_RANGE_LABELS[range]}</Text> : <Text dimColor={true}>{DATE_RANGE_LABELS[range]}</Text>}</Text>);
    $[0] = dateRange;
    $[1] = t1;
  } else {
    t1 = $[1];
  ...
  return <Box flexDirection="column" marginTop={1}>
      ...
      <Box flexDirection="row" gap={4} marginBottom={1}>
        ...
        <Box flexDirection="column" width={28}>
          <Text wrap="truncate">
            Total tokens:{' '}
            <Text color="rash">{formatNumber(totalTokens)}</Text>
          </Text>
        </Box>
      </Box>
      ...
      <Box flexDirection="row" gap={4}>
        <Box flexDirection="column" width={28}>
          <Text wrap="truncate">
            Sessions:{' '}
            <Text color="rash">{formatNumber(stats.totalSessions)}</Text>
          </Text>
        </Box>
        <Box flexDirection="column" width={28}>
      ...
      <Box flexDirection="row" gap={4}>
        <Box flexDirection="column" width={28}>
          <Text wrap="truncate">
            Active days: <Text color="rash">{stats.activeDays}</Text>
      ...
      <Box flexDirection="row" gap={4}>
        <Box flexDirection="column" width={28}>
          {stats.peakActivityDay && <Text wrap="truncate">
              Most active day:{' '}
              <Text color="rash">{formatPeakDay(stats.peakActivityDay)}</Text>
            </Text>}
        </Box>
        <Box flexDirection="column" width={28}>
          <Text wrap="truncate">
      ...
      {shotStatsData && <>
          ...
          <Box flexDirection="row" gap={4}>
            <Box flexDirection="column" width={28}>
              <Text wrap="truncate">
                ...
                <Text color="rash">{shotStatsData.buckets[0]!.count}</Text>
            ...
            <Box flexDirection="column" width={28}>
              <Text wrap="truncate">
                ...
                <Text color="rash">{shotStatsData.buckets[1]!.count}</Text>
          ...
          <Box flexDirection="row" gap={4}>
            <Box flexDirection="column" width={28}>
              <Text wrap="truncate">
                ...
                <Text color="rash">{shotStatsData.buckets[2]!.count}</Text>
            ...
            <Box flexDirection="column" width={28}>
              <Text wrap="truncate">
                ...
                <Text color="rash">{shotStatsData.buckets[3]!.count}</Text>
          ...
          <Box flexDirection="row" gap={4}>
            <Box flexDirection="column" width={28}>
              <Text wrap="truncate">
                Avg/session:{' '}
                <Text color="rash">{shotStatsData.avgShots}</Text>
              </Text>
            </Box>
          </Box>
        </>}
...
function generateFunFactoid(stats: RashCodeStats, totalTokens: number): string {
...
async function handleScreenshot(stats: RashCodeStats, activeTab: 'Overview' | 'Models', setStatus: (status: string | null) => void): Promise<void> {
  setStatus('copying…');
...
function renderStatsToAnsi(stats: RashCodeStats, activeTab: 'Overview' | 'Models'): string {
  const lines: string[] = [];
...
function renderOverviewToAnsi(stats: RashCodeStats): string[] {
  const lines: string[] = [];
  const theme = getTheme(resolveThemeSetting(getGlobalConfig().theme));
  const h = (text: string) => applyColor(text, theme.rash as Color);
...
function renderModelsToAnsi(stats: RashCodeStats): string[] {
  const lines: string[] = [];
  const modelEntries = Object.entries(stats.modelUsage).sort(([, a], [, b]) => b.inputTokens + b.outputTokens - (a.inputTokens + a.outputTokens));
  if (modelEntries.length === 0) {
