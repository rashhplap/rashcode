...
export function Passes({
  ...
  useEffect(() => {
    async function loadPassesData() {
      try {
        ...
        const campaign = eligibilityData.referral_code_details?.campaign ?? 'rash_code_guest_pass';
  ...
  const renderTicket = (pass: PassStatus) => {
    ...
    return <Box key={pass.passNumber} flexDirection="column" marginRight={1}>
        ...
        <Text>
          {' ) CC '}
          <Text color="rash">{TEARDROP_ASTERISK}</Text>
          {' ┊ ( '}
        </Text>
        <Text>{'└──────────┘'}</Text>
      </Box>;
  ...
  return <Pane>
      <Box flexDirection="column" gap={1}>
        ...
        <Box flexDirection="column" marginLeft={2}>
          <Text dimColor>
            {referrerReward ? `Share a free week of Rash Code with friends. If they love it and subscribe, you'll get ${formatCreditAmount(referrerReward)} of extra usage to keep building. ` : 'Share a free week of Rash Code with friends. '}
            <Link url={referrerReward ? 'https://support.rash.com/en/articles/13456702-rash-code-guest-passes' : 'https://support.rash.com/en/articles/12875061-rash-code-guest-passes'}>
              Terms apply.
            </Link>
          </Text>
        </Box>
