...
export function MCPListPanel(t0) {
  ...
  let items;
  if ($[9] !== agentServers || $[10] !== rashAiServers || $[11] !== dynamicServers || $[12] !== serversByScope) {
    items = [];
    ...
    for (const server_0 of rashAiServers) {
      items.push({
        type: "server",
        server: server_0
      });
  ...
  let t23;
  if ($[51] !== rashAiServers || $[52] !== renderServerItem) {
    t23 = rashAiServers.length > 0 && <Box flexDirection="column" marginBottom={1}><Box paddingLeft={2}><Text bold={true}>rash.ai</Text></Box>{rashAiServers.map(server_5 => renderServerItem(server_5))}</Box>;
    $[51] = rashAiServers;
    $[52] = renderServerItem;
    $[53] = t23;
  } else {
  ...
  if ($[60] !== hasFailedClients) {
    t26 = hasFailedClients && <Text dimColor={true}>{debugMode ? "\u203B Error logs shown inline with --debug" : "\u203B Run rash --debug to see error logs"}</Text>;
    $[60] = hasFailedClients;
    $[61] = t26;
  } else {
    t26 = $[61];
  ...
  if ($[62] === Symbol.for("react.memo_cache_sentinel")) {
    t27 = <Text dimColor={true}><Link url="https://code.rash.com/docs/en/mcp">https://code.rash.com/docs/en/mcp</Link>{" "}for help</Text>;
    $[62] = t27;
  } else {
    t27 = $[62];
  }
...
function _temp2(s_0) {
  return s_0.client.config.type === "rashai-proxy";
}
function _temp(s) {
  return s.client.config.type !== "rashai-proxy";
}
