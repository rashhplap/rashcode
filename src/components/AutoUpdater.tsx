...
import { installOrUpdateRashPackage, localInstallationExists } from '../utils/localInstaller.js';
...
export function AutoUpdater({
  ...
  const checkForUpdates = React.useCallback(async () => {
    ...
    if (!isDisabled && currentVersion && latestVersion && !gte(currentVersion, latestVersion) && !shouldSkipVersion(latestVersion)) {
      ...
      if (installationType === 'npm-local') {
        ...
        updateMethod = 'local';
        installStatus = await installOrUpdateRashPackage(channel);
  ...
  return <Box flexDirection="row" gap={1}>
      ...
      {(autoUpdaterResult?.status === 'install_failed' || autoUpdaterResult?.status === 'no_permissions') && <Text color="error" wrap="truncate">
          ✗ Auto-update failed &middot; Try <Text bold>rash doctor</Text> or{' '}
          <Text bold>
            {hasLocalInstall ? `cd ~/.rash/local && npm update ${MACRO.PACKAGE_URL}` : `npm i -g ${MACRO.PACKAGE_URL}`}
          </Text>
        </Text>}
    </Box>;
}
