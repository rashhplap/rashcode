...
export function formatPreconditionError(error: BackgroundRemoteSessionPrecondition): string {
  switch (error.type) {
    case 'not_logged_in':
      return 'Please run /login and sign in with your Rash.ai account (not Console).';
    case 'no_remote_environment':
      return 'No cloud environment available. Set one up at https://rash.ai/code/onboarding?magic=env-setup';
    ...
    case 'github_app_not_installed':
      return 'The Rash GitHub app must be installed on this repository first.\nhttps://github.com/apps/rash/installations/new';
    case 'policy_blocked':
 ...
 *   {type:'system', subtype:'hook_progress', stdout:'...'}. Rash never
 ...
 * rash.ai URL stays a durable record the user can revisit; TTL handles cleanup.
 */
  ...
  const poll = async (): Promise<void> => {
    ...
    try {
      ...
      if (!task || task.status !== 'running') {
        ...
        // Session left alive so the rash.ai URL stays valid — the run_hunt.sh
      ...
      // stableIdle is a prompt-mode completion signal (Rash stops writing
      ...
      if (result || sessionDone || reviewTimedOut) {
        ...
        // the formatted findings as an assistant event, so the rash.ai URL
...
/**
 * RemoteAgentTask - Handles remote Rash.ai session execution.
 *
