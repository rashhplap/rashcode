...
/**
 * Returns a rash config directory path for a given source.
 */
export function getSkillsPath(
  ...
  switch (source) {
    case 'policySettings':
      return join(getManagedFilePath(), '.rash', dir)
    case 'userSettings':
      return join(getRashConfigHomeDir(), dir)
...
/**
 * Parse paths frontmatter from a skill, using the same format as RASH.md rules.
...
export function createSkillCommand({
  ...
  return {
    ...
    async getPromptForCommand(args, toolUseContext) {
      ...
      // Replace ${RASH_SKILL_DIR} with the skill's own directory so bash
      ...
      if (baseDir) {
        ...
        finalContent = finalContent.replace(/\$\{RASH_SKILL_DIR\}/g, skillDir)
      }

      // Replace ${RASH_SESSION_ID} with the current session ID
      ...
      // ${RASH_SKILL_DIR} is meaningless for MCP skills anyway.
      if (loadedFrom !== 'mcp') {
...
export const getSkillDirCommands = memoize(
  async (cwd: string): Promise<Command[]> => {
    const userSkillsDir = join(getRashConfigHomeDir(), 'skills')
    const managedSkillsDir = join(getManagedFilePath(), '.rash', 'skills')
    ...
    const [
      ...
      isEnvTruthy(process.env.RASH_CODE_DISABLE_POLICY_SKILLS)
        ? Promise.resolve([])
...
export async function discoverSkillDirsForPaths(
  ...
  for (const filePath of filePaths) {
    ...
    while (currentDir.startsWith(resolvedCwd + pathSep)) {
      const skillDir = join(currentDir, '.rash', 'skills')
      ...
      if (!dynamicSkillDirs.has(skillDir)) {
        ...
        try {
          ...
          // is gitignored — blocks e.g. node_modules/pkg/.rash/skills from
