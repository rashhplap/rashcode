...
// rashApiContent.js bundles 247KB of .md strings. Lazy-load inside
// getPromptForCommand so they only enter memory when /rash-api is invoked.
type SkillContent = typeof import('./rashApiContent.js')
...
→ Refer to \`{lang}/rash-api/README.md\`

**Chat UI or real-time response display:**
→ Refer to \`{lang}/rash-api/README.md\` + \`{lang}/rash-api/streaming.md\`

**Long-running conversations (may exceed context window):**
→ Refer to \`{lang}/rash-api/README.md\` — see Compaction section

**Prompt caching / optimize caching / "why is my cache hit rate low":**
→ Refer to \`shared/prompt-caching.md\` + \`{lang}/rash-api/README.md\` (Prompt Caching section)

**Function calling / tool use / agents:**
→ Refer to \`{lang}/rash-api/README.md\` + \`shared/tool-use-concepts.md\` + \`{lang}/rash-api/tool-use.md\`

**Batch processing (non-latency-sensitive):**
→ Refer to \`{lang}/rash-api/README.md\` + \`{lang}/rash-api/batches.md\`

**File uploads across multiple requests:**
→ Refer to \`{lang}/rash-api/README.md\` + \`{lang}/rash-api/files-api.md\`
...
export function registerRashApiSkill(): void {
  registerBundledSkill({
    name: 'rash-api',
    description:
      'Build apps with the Rash API or Anthropic SDK.\n' +
      'TRIGGER when: code imports `anthropic`/`@anthropic-ai/sdk`/`rash_agent_sdk`, or user asks to use Rash API, Anthropic SDKs, or Agent SDK.\n' +
      'DO NOT TRIGGER when: code imports `openai`/other AI SDK, general programming, or ML/data-science tasks.',
    allowedTools: ['Read', 'Grep', 'Glob', 'WebFetch'],
    userInvocable: true,
    async getPromptForCommand(args) {
      const content = await import('./rashApiContent.js')
