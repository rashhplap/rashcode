...
const readStoredToken = memoize((): string | undefined => {
  ...
  const envToken = process.env.RASH_TRUSTED_DEVICE_TOKEN
  if (envToken) {
    return envToken
  }
...
export async function enrollTrustedDevice(): Promise<void> {
  try {
    ...
    // If RASH_TRUSTED_DEVICE_TOKEN is set (e.g. by an enterprise wrapper),
    // skip enrollment — the env var takes precedence in readStoredToken() so
    // any enrolled token would be shadowed and never used.
    if (process.env.RASH_TRUSTED_DEVICE_TOKEN) {
      logForDebugging(
        '[trusted-device] RASH_TRUSTED_DEVICE_TOKEN env var is set, skipping enrollment (env var takes precedence)',
      )
      return
    }
    ...
    try {
      response = await axios.post<{
        ...
        { display_name: `Rash Code on ${hostname()} · ${process.platform}` },
        {
          headers: {
