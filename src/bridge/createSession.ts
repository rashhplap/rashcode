 ...
 *
 * Used by both `rash remote-control` (empty session so the user has somewhere to
...
export async function createBridgeSession({
  ...
  const { getRashAIOAuthTokens } = await import('../utils/auth.js')
  ...
  const accessToken =
    getAccessToken?.() ?? getRashAIOAuthTokens()?.accessToken
  if (!accessToken) {
...
export async function getBridgeSession(
  ...
  const accessToken =
    opts?.getAccessToken?.() ?? getRashAIOAuthTokens()?.accessToken
  if (!accessToken) {
 ...
 * explicit client action. Both `rash remote-control` (standalone bridge) and the
 ...
 * connection is active, so the title stays in sync on rash.ai/code.
 *
