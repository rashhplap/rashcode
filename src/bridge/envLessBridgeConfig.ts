...
export type EnvLessBridgeConfig = {
  ...
  min_version: string
  // When true, tell users their rash.ai app may be too old to see v2
...
export async function checkEnvLessBridgeMinVersion(): Promise<string | null> {
  ...
  if (cfg.min_version && lt(MACRO.VERSION, cfg.min_version)) {
    return `Your version of Rash Code (${MACRO.VERSION}) is too old for Remote Control.\nVersion ${cfg.min_version} or higher is required. Run \`rash update\` to update.`
  }
  return null
}

/**
 * Whether to nudge users toward upgrading their rash.ai app when a
