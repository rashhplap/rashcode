...
export const BRIDGE_LOGIN_INSTRUCTION =
  'Remote Control is only available with rash.ai subscriptions. Please use `/login` to sign in with your rash.ai account.'

/** Full error printed when `rash remote-control` is run without auth. */
...
export type WorkSecret = {
  ...
  rash_code_args?: Record<string, string> | null
...
/**
 * How `rash remote-control` chooses session working directories.
 ...
 * `metadata.worker_type` at environment registration so rash.ai can filter
 ...
 */
export type BridgeWorkerType = 'rash_code' | 'rash_code_assistant'
...
export type BridgeConfig = {
   ...
   * instead of creating a new one. Used by `rash remote-control
