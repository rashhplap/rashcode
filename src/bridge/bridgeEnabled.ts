 ...
 *
 * Remote Control requires a rash.ai subscription (the bridge auths to CCR
 * with the rash.ai OAuth token). isRashAISubscriber() excludes
 ...
 * (setup-token, RASH_CODE_OAUTH_TOKEN env var, or pre-scope-expansion
...
export async function getBridgeDisabledReason(): Promise<string | null> {
  if (feature('BRIDGE_MODE')) {
    if (!isRashAISubscriber()) {
      return 'Remote Control requires a rash.ai subscription. Run `rash auth login` to sign in with your rash.ai account.'
    }
    if (!hasProfileScope()) {
      return 'Remote Control requires a full-scope login token. Long-lived tokens (from `rash setup-token` or RASH_CODE_OAUTH_TOKEN) are limited to inference-only for security reasons. Run `rash auth login` to use Remote Control.'
    }
    if (!getOauthAccountInfo()?.organizationUuid) {
      return 'Unable to determine your organization for Remote Control eligibility. Run `rash auth login` to refresh your account information.'
    }
...
// program, before enableConfigs() runs. isRashAISubscriber() → getGlobalConfig()
...
function isRashAISubscriber(): boolean {
  try {
    return authModule.isRashAISubscriber()
  } catch {
    return false
  }
}
 ...
 * rash.ai frontend routes on `session_*`, while v2 worker endpoints hand out
...
export function checkBridgeMinVersion(): string | null {
  ...
  if (feature('BRIDGE_MODE')) {
    ...
    if (config.minVersion && lt(MACRO.VERSION, config.minVersion)) {
      return `Your version of Rash Code (${MACRO.VERSION}) is too old for Remote Control.\nVersion ${config.minVersion} or higher is required. Run \`rash update\` to update.`
    }
  }
  return null
}
...
export function isCcrMirrorEnabled(): boolean {
  return feature('CCR_MIRROR')
    ? isEnvTruthy(process.env.RASH_CODE_CCR_MIRROR) ||
