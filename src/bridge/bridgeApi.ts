        ...
        (token: string) =>
          axios.post<{
            ...
            `${deps.baseUrl}/v1/environments/bridge`,
            {
              ...
              // Advertise session capacity so rash.ai/code can show
              ...
              // worker_type lets rash.ai filter environments by origin
    ...
    case 403:
      throw new BridgeFatalError(
        isExpiredErrorType(errorType)
          ? 'Remote Control session has expired. Please restart with `rash remote-control` or /remote-control.'
