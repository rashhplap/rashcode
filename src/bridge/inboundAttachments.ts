 ...
 * (oauth-authed, same store), write to ~/.rash/uploads/{sessionId}/, and
 * return @path refs to prepend. Rash's Read tool takes it from there.
 *
 * Best-effort: any failure (no token, network, non-2xx, disk) logs debug and
 * skips that attachment. The message still reaches Rash, just without @path.
 */
...
function uploadsDir(): string {
  return join(getRashConfigHomeDir(), 'uploads', getSessionId())
}
...
async function resolveOne(att: InboundAttachment): Promise<string | undefined> {
  ...
  try {
    ...
    // RASH_CODE_CUSTOM_OAUTH_URL — keep it inside the try so a bad
