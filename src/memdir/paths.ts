 ...
 *   1. RASH_CODE_DISABLE_AUTO_MEMORY env var (1/true → OFF, 0/false → ON)
 *   2. RASH_CODE_SIMPLE (--bare) → OFF
 *   3. CCR without persistent storage → OFF (no RASH_CODE_REMOTE_MEMORY_DIR)
 *   4. autoMemoryEnabled in settings.json (supports project-level opt-out)
 *   5. Default: enabled
 */
export function isAutoMemoryEnabled(): boolean {
  const envVal = process.env.RASH_CODE_DISABLE_AUTO_MEMORY
  ...
  if (
    ...
    !process.env.RASH_CODE_REMOTE_MEMORY_DIR
  ) {
    return false
  }
 ...
 * Resolution order:
 *   1. RASH_CODE_REMOTE_MEMORY_DIR env var (explicit override, set in CCR)
 *   2. ~/.rash (default config home)
 */
export function getMemoryBaseDir(): string {
  if (process.env.RASH_CODE_REMOTE_MEMORY_DIR) {
    return process.env.RASH_CODE_REMOTE_MEMORY_DIR
  }
  return getRashConfigHomeDir()
}
...
function getAutoMemPathOverride(): string | undefined {
  return validateMemoryPath(
    process.env.RASH_COWORK_MEMORY_PATH_OVERRIDE,
    false,
  )
}
 ...
 *
 * SECURITY: projectSettings (.rash/settings.json committed to the repo) is
...
/**
 * Check if RASH_COWORK_MEMORY_PATH_OVERRIDE is set to a valid override.
 ...
 * same repo share one auto-memory directory (anthropics/rash-code#24382).
 */
 ...
 *
 * Resolution order:
 *   1. RASH_COWORK_MEMORY_PATH_OVERRIDE env var (full-path override, used by Cowork)
 ...
 * env vars / settings.json / RASH_CONFIG_DIR are session-stable in
 ...
 *
 * When RASH_COWORK_MEMORY_PATH_OVERRIDE is set, this matches against the
