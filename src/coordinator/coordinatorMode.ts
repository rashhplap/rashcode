...
export function isCoordinatorMode(): boolean {
  if (feature('COORDINATOR_MODE')) {
    return isEnvTruthy(process.env.RASH_CODE_COORDINATOR_MODE)
  }
  return false
}
...
export function matchSessionMode(
  ...
  if (sessionIsCoordinator) {
    process.env.RASH_CODE_COORDINATOR_MODE = '1'
  } else {
    delete process.env.RASH_CODE_COORDINATOR_MODE
  }
...
export function getCoordinatorUserContext(
  ...
  const workerTools = isEnvTruthy(process.env.RASH_CODE_SIMPLE)
...
export function getCoordinatorSystemPrompt(): string {
  const workerCapabilities = isEnvTruthy(process.env.RASH_CODE_SIMPLE)
    ? 'Workers have access to Bash, Read, and Edit tools, plus MCP tools from configured MCP servers.'
    : 'Workers have access to standard tools, MCP tools from configured MCP servers, and project skills via the Skill tool. Delegate skill invocations (e.g. /commit, /verify) to workers.'

  return `You are Rash Code, an AI assistant that orchestrates software engineering tasks across multiple workers.
...
2. Precise git operation: "Create a new branch from main called 'fix/session-expiry'. Cherry-pick only commit abc123 onto it. Push and create a draft PR targeting main. Add anthropics/rash-code as reviewer. Report the PR URL."
