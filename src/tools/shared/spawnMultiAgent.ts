...
/**
 * Ensures a team file exists on disk. If it doesn't (e.g. when a non-Rash
 ...
 * When outside tmux: Creates a rash-swarm session with all teammates in a tiled layout.
 */
async function handleSpawnSplitPane(
  ...
  // - Outside both: creates rash-swarm session with tiled teammates
  ...
  // Build the command to spawn Rash Code with teammate identity
  // Note: We spawn without a prompt - initial instructions are sent via mailbox
  const binaryPath = getTeammateCommand()

  // Build teammate identity CLI args (replaces RASH_CODE_* env vars)
  const teammateArgs = [
  ...
  // Includes RASHCODE, RASH_CODE_EXPERIMENTAL_AGENT_TEAMS, and API provider vars.
...
/**
 * Handle spawn operation - creates a new Rash Code instance.
