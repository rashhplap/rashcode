...
function isPathAllowed(
  ...
  // This MUST come before checkPathSafetyForAutoEdit since .rash is a dangerous directory
  // and internal editable paths live under ~/.rash/ — matching the ordering in
  ...
  // have explicitly configured writable directories (e.g. /tmp/rash/) —
...
function checkPathConstraintsForStatement(
  ...
  //   Set-Location ./.rash; Set-Content ./settings.json '...'
  // Validator sees ./settings.json → /project/settings.json (not a config file).
  // Runtime writes /project/.rash/settings.json (Rash's permission config).
  //
  // ALTERNATIVE APPROACH (rejected): simulate cwd through the statement chain
  // — after `Set-Location ./.rash`, validate subsequent statements with
  // cwd='./.rash'. This would be more permissive but requires careful
  // handling of:
      ...
      if (!allowed) {
        ...
        const message =
          ...
          decisionReason?.type === 'safetyCheck'
            ...
            : `${canonical} targeting '${resolvedPath}' was blocked. For security, Rash Code may only access files in the allowed working directories for this session: ${dirListStr}.`
