...
export const ThinkingAdaptiveSchema = lazySchema(() =>
  z
    ...
    .describe('Rash decides when and how much to think (Opus 4.6+).'),
)
...
export const ThinkingConfigSchema = lazySchema(() =>
  z
    ...
    .describe(
      "Controls Rash's thinking/reasoning behavior. When set, takes precedence over the deprecated maxThinkingTokens.",
    ),
)
...
export const McpRashAIProxyServerConfigSchema = lazySchema(() =>
  z.object({
    type: z.literal('rashai-proxy'),
    url: z.string(),
    id: z.string(),
  }),
)

// Broader config type for status responses (includes rashai-proxy which is output-only)
export const McpServerStatusConfigSchema = lazySchema(() =>
  z.union([
    McpServerConfigForProcessTransportSchema(),
    McpRashAIProxyServerConfigSchema(),
  ]),
)
  ...
  z
    .object({
      ...
      scope: z
        ...
        .optional()
        .describe(
          'Configuration scope (e.g., project, user, local, rashai, managed)',
        ),
      tools: z
        .array(
          z.object({
      ...
      capabilities: z
        ...
        .optional()
        .describe(
          "@internal Server capabilities (available when connected). experimental['rash/channel'] is only present if the server's plugin is on the approved channels allowlist — use its presence to decide whether to show an Enable-channel prompt.",
        ),
    })
  ...
  z
    .object({
      ...
      supportsAdaptiveThinking: z
        ...
        .optional()
        .describe(
          'Whether this model supports adaptive thinking (Rash decides when and how much to think)',
        ),
      supportsFastMode: z
        .boolean()
        .optional()
  ...
  z
    .object({
      ...
      model: z
        ...
        .optional()
        .describe(
          "Model alias (e.g. 'sonnet', 'opus', 'haiku') or full model ID (e.g. 'rash-opus-4-5'). If omitted or 'inherit', uses the main model",
        ),
      ...
      memory: z
        ...
        .optional()
        .describe(
          "Scope for auto-loading agent memory files. 'user' - ~/.rash/agent-memory/<agentType>/, 'project' - .rash/agent-memory/<agentType>/, 'local' - .rash/agent-memory-local/<agentType>/",
        ),
      effort: z
  ...
  z
    ...
    .describe(
      'Source for loading filesystem-based settings. ' +
        "'user' - Global user settings (~/.rash/settings.json). " +
        "'project' - Project settings (.rash/settings.json). " +
        "'local' - Local settings (.rash/settings.local.json).",
    ),
)
...
export const SDKRateLimitInfoSchema = lazySchema(() =>
  z
    ...
    .describe('Rate limit information for rash.ai subscription users.'),
)
