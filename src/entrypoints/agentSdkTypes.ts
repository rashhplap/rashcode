/**
 * Main entrypoint for Rash Code Agent SDK types.
 *
 ...
 *
 * If your SDK MCP calls will run longer than 60s, override RASH_CODE_STREAM_CLOSE_TIMEOUT
 */
...
/**
 * A scheduled task from `<dir>/.rash/scheduled_tasks.json`.
 * @internal
 */
...
/**
 * Watch `<dir>/.rash/scheduled_tasks.json` and yield events as tasks fire.
 *
...
/**
 * A user message typed on rash.ai, extracted from the bridge WS.
 * @internal
 */
...
/**
 * Hold a rash.ai remote-control bridge connection from a daemon process.
 *
 ...
 * rash.ai keeps the same session. Contrast with `query.enableRemoteControl`
 * which puts the WS in the CHILD process (dies with the agent).
 *
 * Pipe `query()` yields through `write()` + `sendResult()`. Read
 * `inboundPrompts()` (user typed on rash.ai) into `query()`'s input
