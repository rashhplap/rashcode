...
if (process.env.RASH_CODE_REMOTE === 'true') {
...
// eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
if (feature('ABLATION_BASELINE') && process.env.RASH_CODE_ABLATION_BASELINE) {
  for (const k of ['RASH_CODE_SIMPLE', 'RASH_CODE_DISABLE_THINKING', 'DISABLE_INTERLEAVED_THINKING', 'DISABLE_COMPACT', 'DISABLE_AUTO_COMPACT', 'RASH_CODE_DISABLE_AUTO_MEMORY', 'RASH_CODE_DISABLE_BACKGROUND_TASKS']) {
    // eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
    process.env[k] ??= '1';
  }
}
...
function getProviderValidationError(
  ...
  const useOpenAI = isEnvTruthy(env.RASH_CODE_USE_OPENAI)
  const useGithub = isEnvTruthy(env.RASH_CODE_USE_GITHUB)

  if (isEnvTruthy(env.RASH_CODE_USE_GEMINI)) {
    if (!(env.GEMINI_API_KEY ?? env.GOOGLE_API_KEY)) {
      return 'GEMINI_API_KEY is required when RASH_CODE_USE_GEMINI=1.'
    }
    return null
  }
  ...
  if (useGithub && !useOpenAI) {
    ...
    if (!token) {
      return 'GITHUB_TOKEN or GH_TOKEN is required when RASH_CODE_USE_GITHUB=1.'
    }
    return null
  }
  ...
  if (!env.OPENAI_API_KEY && !isLocalProviderUrl(request.baseUrl)) {
    ...
    return 'OPENAI_API_KEY is required when RASH_CODE_USE_OPENAI=1 and OPENAI_BASE_URL is not local.'
  }
...
async function main(): Promise<void> {
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.DISPLAY_VERSION ?? MACRO.VERSION} (Open Rash)`);
    return;
  }
  ...
  if (process.argv[2] === '--rash-in-chrome-mcp') {
    profileCheckpoint('cli_rash_in_chrome_mcp_path');
    const {
      runRashInChromeMcpServer
    } = await import('../utils/rashInChrome/mcpServer.js');
    await runRashInChromeMcpServer();
    return;
  } else if (process.argv[2] === '--chrome-native-host') {
    profileCheckpoint('cli_chrome_native_host_path');
    const {
      runChromeNativeHost
    } = await import('../utils/rashInChrome/chromeNativeHost.js');
  ...
  // Fast-path for `rash remote-control` (also accepts legacy `rash remote` / `rash sync` / `rash bridge`):
  ...
  // Fast-path for `rash daemon [subcommand]`: long-running supervisor.
  ...
  // Fast-path for `rash ps|logs|attach|kill` and `--bg`/`--background`.
  // Session management against the ~/.rash/sessions/ registry. Flag
  ...
  // Fast-path for `rash environment-runner`: headless BYOC runner.
  // feature() must stay inline for build-time dead code elimination.
  ...
  // Fast-path for `rash self-hosted-runner`: headless self-hosted-runner
  ...
  if (process.env.rashcode_ENABLE_EARLY_INPUT === '1') {
    const {
