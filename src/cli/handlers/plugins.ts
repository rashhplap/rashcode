 ...
 * These are dynamically imported only when `rash plugin *` or `rash plugin marketplace *` runs.
 */
...
export async function pluginValidateHandler(
  ...
  try {
    ...
    // If this is a plugin manifest located inside a .rash-plugin directory,
    ...
    if (result.fileType === 'plugin') {
      ...
      if (basename(manifestDir) === '.rash-plugin') {
...
export async function pluginListHandler(options: {
  ...
  if (pluginIds.length === 0 && inlinePlugins.length === 0) {
    ...
    if (inlineLoadErrors.length === 0) {
      cliOk(
        'No plugins installed. Use `rash plugin install` to install a plugin.',
      )
    }
  }
