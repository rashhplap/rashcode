...
export class StructuredIO {
   ...
   * Used by the bridge to feed permission responses from rash.ai into the
   * SDK permission flow.
   *
   ...
   * stale permission prompt on rash.ai when the SDK consumer wins the race.
   */
  ...
  private async processLine(
    ...
    try {
      ...
      if (message.type === 'update_environment_variables') {
        ...
        // (RASH_CODE_SESSION_ACCESS_TOKEN) which must be readable
      ...
      if (message.type === 'control_response') {
        ...
        // request, so it can cancel the stale permission prompt on rash.ai.
        if (
