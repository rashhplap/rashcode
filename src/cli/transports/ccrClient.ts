 ...
 * - Epoch management: reads worker_epoch from RASH_CODE_WORKER_EPOCH env var
...
export class CCRClient {
   ...
   * (RASH_CODE_SESSION_ACCESS_TOKEN env var). Callers managing multiple
  ...
  constructor(
    ...
    opts?: {
       ...
       * RASH_CODE_SESSION_ACCESS_TOKEN (single-session callers — REPL,
   ...
   *    RASH_CODE_WORKER_EPOCH (set by env-manager / bridge spawner)
  ...
  async initialize(epoch?: number): Promise<Record<string, unknown> | null> {
    ...
    if (epoch === undefined) {
      const rawEpoch = process.env.RASH_CODE_WORKER_EPOCH
