export const PRODUCT_URL = 'https://rash.com/rash-code'

// Rash Code Remote session URLs
export const RASH_AI_BASE_URL = 'https://rash.ai'
export const RASH_AI_STAGING_BASE_URL = 'https://rash-ai.staging.ant.dev'
export const RASH_AI_LOCAL_BASE_URL = 'http://localhost:4000'
...
/**
 * Get the base URL for Rash AI based on environment.
 */
export function getRashAiBaseUrl(
  sessionId?: string,
  ingressUrl?: string,
): string {
  if (isRemoteSessionLocal(sessionId, ingressUrl)) {
    return RASH_AI_LOCAL_BASE_URL
  }
  if (isRemoteSessionStaging(sessionId, ingressUrl)) {
    return RASH_AI_STAGING_BASE_URL
  }
  return RASH_AI_BASE_URL
}
 ...
 * endpoints (/v1/code/sessions/{id}/worker/*) want `cse_*` but the rash.ai
...
export function getRemoteSessionUrl(
  ...
  const baseUrl = getRashAiBaseUrl(compatId, ingressUrl)
