...
import { accumulateUsage, updateUsage } from 'src/services/api/rash.js'
...
export class QueryEngine {
  ...
  async *submitMessage(
    ...
    // RASH_COWORK_MEMORY_PATH_OVERRIDE, inject the memory-mechanics prompt.
    // The env var is an explicit opt-in signal — the caller has wired up
    // a memory directory and needs Rash to know how to use it (which
    ...
    if (persistSession && messagesFromUserInput.length > 0) {
      ...
      if (isBareMode()) {
        ...
        if (
          isEnvTruthy(process.env.RASH_CODE_EAGER_FLUSH) ||
          isEnvTruthy(process.env.RASH_CODE_IS_COWORK)
        ) {
    ...
    // ref-tracked plugins. CCR populates the cache via RASH_CODE_SYNC_PLUGIN_INSTALL
    // (headlessPluginInstall) or RASH_CODE_PLUGIN_SEED_DIR before this runs;
    ...
    for await (const message of query({
      ...
      if (
        ...
        // If the SDK subprocess restarts before then (rash-desktop kills
        ...
        if (persistSession) {
          // Fire-and-forget for assistant messages. rash.ts yields one
      ...
      switch (message.type) {
        ...
        case 'stream_event':
          ...
          if (message.event.type === 'message_delta') {
            ...
            // real value only arrives here (see rash.ts message_delta
...
/**
 * Sends a single prompt to the Rash API and returns the response.
 * Assumes that rash is being used non-interactively -- will not
