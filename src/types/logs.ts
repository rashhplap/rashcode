...
export type SerializedMessage = Message & {
  cwd: string
  userType: string
  entrypoint?: string // RASH_CODE_ENTRYPOINT — distinguishes cli/sdk-ts/sdk-py/etc.
  sessionId: string
  timestamp: string
  version: string
  gitBranch?: string
 ...
 * `rash ps` can show something more useful than the last user prompt
...
/**
 * Per-file attribution state tracking Rash's character contributions.
 */
export type FileAttributionState = {
  contentHash: string // SHA-256 hash of file content
  rashContribution: number // Characters written by Rash
  mtime: number // File modification time
}

/**
 * Attribution snapshot message stored in session transcript.
 * Tracks character-level contributions by Rash for commit attribution.
 */
