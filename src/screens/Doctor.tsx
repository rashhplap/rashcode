...
import { getRashConfigHomeDir } from 'src/utils/envUtils.js';
...
export function Doctor(t0) {
  ...
  if ($[5] === Symbol.for("react.memo_cache_sentinel")) {
    const envVars = [{
      ...
      name: "RASH_CODE_MAX_OUTPUT_TOKENS",
      ...getModelMaxOutputTokens("rash-opus-4-6")
    }];
  ...
  if ($[6] !== agentDefinitions || $[7] !== toolPermissionContext || $[8] !== tools) {
    t5 = () => {
      ...
      (async () => {
        const userAgentsDir = join(getRashConfigHomeDir(), "agents");
        const projectAgentsDir = join(getOriginalCwd(), ".rash", "agents");
        const {
          activeAgents,
          allAgents,
          failedFiles
        ...
        if (isPidBasedLockingEnabled()) {
          const locksDir = join(getXDGStateHome(), "rash", "locks");
  ...
  if ($[11] !== onDone) {
    t7 = () => {
      onDone("Rash Code diagnostics dismissed", {
        display: "system"
      });
    };
    $[11] = onDone;
  ...
  if ($[73] !== contextWarnings) {
    t39 = contextWarnings && (contextWarnings.rashMdWarning || contextWarnings.agentWarning || contextWarnings.mcpWarning) && <Box flexDirection="column"><Text bold={true}>Context Usage Warnings</Text>{contextWarnings.rashMdWarning && <><Text>└{" "}<Text color="warning">{figures.warning} {contextWarnings.rashMdWarning.message}</Text></Text><Text>{"  "}└ Files:</Text>{contextWarnings.rashMdWarning.details.map(_temp16)}</>}{contextWarnings.agentWarning && <><Text>└{" "}<Text color="warning">{figures.warning} {contextWarnings.agentWarning.message}</Text></Text><Text>{"  "}└ Top contributors:</Text>{contextWarnings.agentWarning.details.map(_temp17)}</>}{contextWarnings.mcpWarning && <><Text>└{" "}<Text color="warning">{figures.warning} {contextWarnings.mcpWarning.message}</Text></Text><Text>{"  "}└ MCP servers:</Text>{contextWarnings.mcpWarning.details.map(_temp18)}</>}</Box>;
    $[73] = contextWarnings;
    $[74] = t39;
  } else {
    t39 = $[74];
