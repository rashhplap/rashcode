...
/**
 * Gets additional files from Rash config directories
 */
async function getRashConfigFiles(cwd: string): Promise<string[]> {
  const markdownFileArrays = await Promise.all(
    RASH_CONFIG_DIRECTORIES.map(subdir =>
