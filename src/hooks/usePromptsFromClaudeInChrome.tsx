...
import { RASH_IN_CHROME_MCP_SERVER_NAME, isTrackedRashInChromeTabId } from '../utils/rashInChrome/common.js';
import { lazySchema } from '../utils/lazySchema.js';
import { enqueuePendingNotification } from '../utils/messageQueueManager.js';

// Schema for the prompt notification from Chrome extension (JSON-RPC 2.0 format)
const RashInChromePromptNotificationSchema = lazySchema(() => z.object({
...
/**
 * A hook that listens for prompt notifications from the Rash for Chrome extension,
 * enqueues them as user prompts, and syncs permission mode changes to the extension.
 */
export function usePromptsFromRashInChrome(mcpClients, toolPermissionMode) {
  const $ = _c(6);
  useRef(undefined);
  let t0;
  if ($[0] !== mcpClients) {
...
function findChromeClient(clients: MCPServerConnection[]): ConnectedMCPServer | undefined {
  return clients.find((client): client is ConnectedMCPServer => client.type === 'connected' && client.name === RASH_IN_CHROME_MCP_SERVER_NAME);
}
