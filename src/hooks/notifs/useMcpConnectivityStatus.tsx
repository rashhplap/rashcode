...
import { hasRashAiMcpEverConnected } from '../../services/mcp/rashai.js';
  ...
  if ($[0] !== addNotification || $[1] !== mcpClients) {
    t2 = () => {
      try {
        ...
        const failedRashAiClients = mcpClients.filter(_temp2);
        const needsAuthLocalServers = mcpClients.filter(_temp3);
        const needsAuthRashAiServers = mcpClients.filter(_temp4);
        if (failedLocalClients.length === 0 && failedRashAiClients.length === 0 && needsAuthLocalServers.length === 0 && needsAuthRashAiServers.length === 0) {
          return;
        }
        ...
        if (failedRashAiClients.length > 0) {
          addNotification({
            key: "mcp-rashai-failed",
            jsx: <><Text color="error">{failedRashAiClients.length} rash.ai{" "}{failedRashAiClients.length === 1 ? "connector" : "connectors"}{" "}unavailable</Text><Text dimColor={true}> · /mcp</Text></>,
            priority: "medium"
          });
        }
        ...
        if (needsAuthRashAiServers.length > 0) {
          addNotification({
            key: "mcp-rashai-needs-auth",
            jsx: <><Text color="warning">{needsAuthRashAiServers.length} rash.ai{" "}{needsAuthRashAiServers.length === 1 ? "connector needs" : "connectors need"}{" "}auth</Text><Text dimColor={true}> · /mcp</Text></>,
            priority: "medium"
          });
        }
      } catch (error) {
...
function _temp4(client_2) {
  return client_2.type === "needs-auth" && client_2.config.type === "rashai-proxy" && hasRashAiMcpEverConnected(client_2.name);
}
function _temp3(client_1) {
  return client_1.type === "needs-auth" && client_1.config.type !== "rashai-proxy";
}
function _temp2(client_0) {
  return client_0.type === "failed" && client_0.config.type === "rashai-proxy" && hasRashAiMcpEverConnected(client_0.name);
}
function _temp(client) {
  return client.type === "failed" && client.config.type !== "sse-ide" && client.config.type !== "ws-ide" && client.config.type !== "rashai-proxy";
}
