...
async function _temp2() {
  ...
  const subscriptionType = await getExistingRashSubscription();
  ...
  return {
    ...
    jsx: <Text color="suggestion">Use your existing Rash {subscriptionType} plan with Rash Code<Text color="text" dimColor={true}>{" "}· /login to activate</Text></Text>,
    priority: "low"
  };
}
...
async function getExistingRashSubscription(): Promise<'Max' | 'Pro' | null> {
  ...
  if (profile.account.has_rash_max) {
    return 'Max';
  }
  if (profile.account.has_rash_pro) {
    return 'Pro';
  }
  return null;
}
