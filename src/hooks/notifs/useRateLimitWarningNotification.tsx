...
import { getRateLimitWarning, getUsingOverageText } from 'src/services/rashAiLimits.js';
import { useRashAiLimits } from 'src/services/rashAiLimitsHook.js';
import { getSubscriptionType } from 'src/utils/auth.js';
import { hasRashAiBillingAccess } from 'src/utils/billing.js';
...
export function useRateLimitWarningNotification(model) {
  ...
  let t0;
  if ($[0] !== rashAiLimits || $[1] !== model) {
    t0 = getRateLimitWarning(rashAiLimits, model);
    $[0] = rashAiLimits;
    $[1] = model;
    $[2] = t0;
  } else {
  ...
  let t1;
  if ($[3] !== rashAiLimits) {
    t1 = getUsingOverageText(rashAiLimits);
    $[3] = rashAiLimits;
    $[4] = t1;
  } else {
    t1 = $[4];
  ...
  let t4;
  let t5;
  if ($[7] !== addNotification || $[8] !== rashAiLimits.isUsingOverage || $[9] !== hasShownOverageNotification || $[10] !== usingOverageText) {
    t4 = () => {
      if (getIsRemoteMode()) {
        return;
      }
      if (rashAiLimits.isUsingOverage && !hasShownOverageNotification && (!isTeamOrEnterprise || hasBillingAccess)) {
        addNotification({
          key: "limit-reached",
        ...
        if (!rashAiLimits.isUsingOverage && hasShownOverageNotification) {
          setHasShownOverageNotification(false);
        }
      }
    };
    t5 = [rashAiLimits.isUsingOverage, usingOverageText, hasShownOverageNotification, addNotification, hasBillingAccess, isTeamOrEnterprise];
    $[7] = addNotification;
    $[8] = rashAiLimits.isUsingOverage;
