...
export type ToolUseContext = {
  ...
  /**
   * RASH.md paths already injected as nested_memory attachments this
   ...
   * re-inject the same RASH.md dozens of times.
   */
...
> = {
   ...
   * or unprefixed (RASH_AGENT_SDK_MCP_NO_PREFIX mode).
   */
  mcpInfo?: { serverName: string; toolName: string }
  readonly name: string
  /**
   * Maximum size in characters for tool result before it gets persisted to disk.
   * When exceeded, the result is saved to a file and Rash receives a preview
