...
3. **Installed verification packages** (for web apps)
   ...
   - Check MCP configuration (.mcp.json) for browser automation tools:
     ...
     - Rash Chrome Extension MCP (browser-use via Rash's Chrome extension)
...
2. **If NO browser automation tools are detected**, ask if they want to install/configure one:
   ...
   - Options to offer:
     ...
     - **Rash Chrome Extension** - Uses the Rash Chrome extension for browser interaction (requires the extension installed in Chrome)
...
4. **If user chooses Chrome DevTools MCP or Rash Chrome Extension**:
   ...
   - For Rash Chrome Extension, inform them they need the extension installed from the Chrome Web Store
...
**All verifier skills are created in the project root's \`.rash/skills/\` directory.** This ensures they are automatically loaded when Rash runs in the project.

Write the skill file to \`.rash/skills/<verifier-name>/SKILL.md\`.
...
1. Where each skill was created (always in \`.rash/skills/\`)
