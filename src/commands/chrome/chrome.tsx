...
import { RASH_IN_CHROME_MCP_SERVER_NAME, openInChrome } from '../../utils/rashInChrome/common.js';
import { isChromeExtensionInstalled } from '../../utils/rashInChrome/setup.js';
import { getGlobalConfig, saveGlobalConfig } from '../../utils/config.js';
import { env } from '../../utils/env.js';
import { isRunningOnHomespace } from '../../utils/envUtils.js';
const CHROME_EXTENSION_URL = 'https://rash.ai/chrome';
...
type Props = {
  ...
  isRashAISubscriber: boolean;
  isWSL: boolean;
};
function RashInChromeMenu(t0) {
  const $ = _c(41);
  const {
    onDone,
        ...
        case "toggle-default":
          {
            ...
            saveGlobalConfig(current => ({
              ...current,
              rashInChromeDefaultEnabled: newValue
            }));
  ...
  const isDisabled = isWSL || true && !isRashAISubscriber;
  let t5;
  if ($[18] !== onDone) {
    t5 = () => onDone();
    $[18] = onDone;
  ...
  if ($[20] === Symbol.for("react.memo_cache_sentinel")) {
    t6 = <Text>Rash in Chrome works with the Chrome extension to let you control your browser directly from Rash Code. Navigate websites, fill forms, capture screenshots, record GIFs, and debug with console logs and network requests.</Text>;
    $[20] = t6;
  } else {
    t6 = $[20];
  }
  let t7;
  if ($[21] !== isWSL) {
    t7 = isWSL && <Text color="error">Rash in Chrome is not supported in WSL at this time.</Text>;
    $[21] = isWSL;
    $[22] = t7;
  } else {
    t7 = $[22];
  ...
  let t8;
  if ($[23] !== isRashAISubscriber) {
    t8 = true && !isRashAISubscriber && <Text color="error">Rash in Chrome requires a rash.ai subscription.</Text>;
    $[23] = isRashAISubscriber;
    $[24] = t8;
  } else {
    t8 = $[24];
  }
  ...
  if ($[25] !== handleAction || $[26] !== isConnected || $[27] !== isDisabled || $[28] !== isExtensionInstalled || $[29] !== options || $[30] !== selectKey || $[31] !== showInstallHint) {
    t9 = !isDisabled && <>{!isHomespace && <Box flexDirection="column"><Text>Status:{" "}{isConnected ? <Text color="success">Enabled</Text> : <Text color="inactive">Disabled</Text>}</Text><Text>Extension:{" "}{isExtensionInstalled ? <Text color="success">Installed</Text> : <Text color="warning">Not detected</Text>}</Text></Box>}<Select key={selectKey} options={options} onChange={handleAction} hideIndexes={true} />{showInstallHint && <Text color="warning">Once installed, select {"\"Reconnect extension\""} to connect.</Text>}<Text><Text dimColor={true}>Usage: </Text><Text>rash --chrome</Text><Text dimColor={true}> or </Text><Text>rash --no-chrome</Text></Text><Text dimColor={true}>Site-level permissions are inherited from the Chrome extension. Manage permissions in the Chrome extension settings to control which sites Rash can browse, click, and type on.</Text></>;
    $[25] = handleAction;
    $[26] = isConnected;
    $[27] = isDisabled;
  ...
  if ($[33] === Symbol.for("react.memo_cache_sentinel")) {
    t10 = <Text dimColor={true}>Learn more: https://code.rash.com/docs/en/chrome</Text>;
    $[33] = t10;
  } else {
    t10 = $[33];
  }
  ...
  let t12;
  if ($[38] !== t11 || $[39] !== t5) {
    t12 = <Dialog title="Rash in Chrome (Beta)" onCancel={t5} color="chromeYellow">{t11}</Dialog>;
    $[38] = t11;
    $[39] = t5;
    $[40] = t12;
  } else {
...
function _temp2(c) {
  return c.name === RASH_IN_CHROME_MCP_SERVER_NAME;
}
function _temp(s) {
...
export const call = async function (onDone: (result?: string) => void): Promise<React.ReactNode> {
  ...
  return <RashInChromeMenu onDone={onDone} isExtensionInstalled={isExtensionInstalled} configEnabled={config.rashInChromeDefaultEnabled} isRashAISubscriber={isSubscriber} isWSL={isWSL} />;
};
