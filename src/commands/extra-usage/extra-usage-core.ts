...
export async function runExtraUsage(): Promise<ExtraUsageResult> {
  ...
  const hasBillingAccess = hasRashAiBillingAccess()

  if (!hasBillingAccess && isTeamOrEnterprise) {
    // Mirror apps/rash-ai useHasUnlimitedOverage(): if overage is enabled
  ...
  const url = isTeamOrEnterprise
    ? 'https://rash.ai/admin-settings/usage'
    : 'https://rash.ai/settings/usage'
