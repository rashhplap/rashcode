  ...
  if ($[0] !== onComplete || $[1] !== path) {
    t1 = () => {
      const runValidation = async function runValidation() {
        if (!path) {
          onComplete("Usage: /plugin validate <path>\n\nValidate a plugin or marketplace manifest file or directory.\n\nExamples:\n  /plugin validate .rash-plugin/plugin.json\n  /plugin validate /path/to/plugin-directory\n  /plugin validate .\n\nWhen given a directory, automatically validates .rash-plugin/marketplace.json\nor .rash-plugin/plugin.json (prefers marketplace if both exist).\n\nOr from the command line:\n  rash plugin validate <path>");
          return;
        }
        ;
        try {
