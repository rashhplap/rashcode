...
import { getAdditionalDirectoriesForRashMd } from '../../bootstrap/state.js'
import {
...
async function getWatchablePaths(): Promise<string[]> {
  ...
  // User skills directory (~/.rash/skills)
  ...
  // User commands directory (~/.rash/commands)
  ...
  // Project skills directory (.rash/skills)
  ...
  // Project commands directory (.rash/commands)
  ...
  for (const dir of getAdditionalDirectoriesForRashMd()) {
    const additionalSkillsPath = platformPath.join(dir, '.rash', 'skills')
    try {
