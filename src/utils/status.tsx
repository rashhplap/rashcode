...
import { getAccountInformation, isRashAISubscriber } from './auth.js';
import { getLargeMemoryFiles, getMemoryFiles, MAX_MEMORY_CHARACTER_COUNT } from './rashmd.js';
import { getDoctorDiagnostic } from './doctorDiagnostic.js';
import { getAWSRegion, getDefaultVertexRegion, isEnvTruthy } from './envUtils.js';
import { getDisplayPath } from './file.js';
import { formatNumber } from './format.js';
import { getIdeClientName, type IDEExtensionInstallationStatus, isJetBrainsIde, toIDEDisplayName } from './ide.js';
import { getRashAiUserDefaultModelDescription, modelDisplayString } from './model/model.js';
...
export function buildAPIProviderProperties(): Property[] {
  ...
  if (apiProvider === 'firstParty') {
    ...
    if (isEnvTruthy(process.env.RASH_CODE_SKIP_BEDROCK_AUTH)) {
      properties.push({
  ...
  if (mtlsConfig) {
    if (mtlsConfig.cert && process.env.RASH_CODE_CLIENT_CERT) {
      properties.push({
        label: 'mTLS client cert',
        value: process.env.RASH_CODE_CLIENT_CERT
      });
    }
    if (mtlsConfig.key && process.env.RASH_CODE_CLIENT_KEY) {
      properties.push({
        label: 'mTLS client key',
        value: process.env.RASH_CODE_CLIENT_KEY
      });
    }
  }
  return properties;
...
export function getModelDisplayLabel(mainLoopModel: string | null): string {
  ...
  if (mainLoopModel === null && isRashAISubscriber()) {
    const description = getRashAiUserDefaultModelDescription();
