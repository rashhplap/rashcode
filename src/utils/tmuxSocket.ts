 ...
 * =====================
 * This module manages an isolated tmux socket for Rash's operations.
 *
 * WHY THIS EXISTS:
 * Without isolation, Rash could accidentally affect the user's tmux sessions.
 * For example, running `tmux kill-session` via the Bash tool would kill the
 * user's current session if they started Rash from within tmux.
 *
 * HOW IT WORKS:
 * 1. Rash creates its own tmux socket: `rash-<PID>` (e.g., `rash-12345`)
 ...
 *    (set in Shell.ts via getRashTmuxEnv())
 *
 * This means ANY tmux command run through Rash - whether via the Tmux tool
 * directly or via Bash - will operate on Rash's isolated socket, NOT the
 * user's tmux session.
 *
 * IMPORTANT: The user's original TMUX env var is NOT used. After socket
 * initialization, getRashTmuxEnv() returns a value that overrides the
...
/**
 * Gets the socket name for Rash's isolated tmux session.
 * Format: rash-<PID>
 */
export function getRashSocketName(): string {
  if (!socketName) {
    socketName = `${RASH_SOCKET_PREFIX}-${process.pid}`
  }
  return socketName
}
 ...
 */
export function getRashSocketPath(): string | null {
  return socketPath
}
 ...
 */
export function setRashSocketInfo(path: string, pid: number): void {
  socketPath = path
  serverPid = pid
}
...
/**
 * Gets the TMUX environment variable value for Rash's isolated socket.
 *
 ...
 * initializing the socket. getRashTmuxEnv() will return null, and
 * Bash commands will run without tmux isolation.
 */
...
async function doInitialize(): Promise<void> {
  ...
  // Pass RASH_CODE_SKIP_PROMPT_HISTORY via -e so it's set in the initial shell environment
  //
  ...
  const result = await execTmux([
    ...
    'base',
    '-e',
    'RASH_CODE_SKIP_PROMPT_HISTORY=true',
    ...(getPlatform() === 'windows'
  ...
  // Set RASH_CODE_SKIP_PROMPT_HISTORY in the tmux GLOBAL environment (-g).
  ...
  // Any Rash Code instance spawned on this socket will inherit this env var,
  ...
  if (pidResult.code === 0) {
    ...
    if (!isNaN(pid)) {
      ...
      setRashSocketInfo(fallbackPath, pid)
      return
    }
    // PID parsing failed
    logForDebugging(
