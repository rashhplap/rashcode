...
function createHttpsProxyAgent(
  ...
  if (isEnvTruthy(process.env.RASH_CODE_PROXY_RESOLVES_HOSTS)) {
 ...
 *   env var is set by `rash ssh` on the remote CLI to route API calls through
...
export function getProxyFetchOptions(opts?: { forAnthropicAPI?: boolean }): {
  ...
  // ANTHROPIC_UNIX_SOCKET tunnels through the `rash ssh` auth proxy, which
