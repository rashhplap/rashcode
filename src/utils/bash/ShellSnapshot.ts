...
/**
 * Generates Rash Code specific snapshot content
 * This content is always included regardless of user configuration
 */
async function getRashCodeSnapshotContent(): Promise<string> {
...
RIPGREP_FUNC_END
  ...
  // consistent fast behavior in Rash's shell.
...
async function getSnapshotScript(
  ...
  // Generate the user content and Rash Code content
  const userContent = configFileExists
    ...
    : !isZsh
      ? // we need to manually force alias expansion in bash - normally `getUserSnapshotContent` takes care of this
        'echo "shopt -s expand_aliases" >> "$SNAPSHOT_FILE"'
      : ''
  const rashCodeContent = await getRashCodeSnapshotContent()
 ...
 *
 * This function is a critical part of Rash CLI's shell integration strategy. It:
 *
 ...
 *
 * This approach allows Rash CLI to execute commands as if they were run in the user's
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with Rash Code defaults only`,
        )
      }
      ...
      const snapshotsDir = join(getRashConfigHomeDir(), 'shell-snapshots')
      ...
      execFile(
        ...
        {
          env: {
            ...((process.env.RASH_CODE_DONT_INHERIT_ENV
              ? {}
        ...
        async (error, stdout, stderr) => {
          if (error) {
            ...
            logForDebugging(`  - Rash home: ${getRashConfigHomeDir()}`)
