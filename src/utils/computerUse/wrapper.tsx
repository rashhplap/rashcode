 ...
 * rule (src/RASH.md): the dispatcher closure must persist across calls so
...
function formatLockHeld(holder: string): string {
  return `Computer use is in use by another Rash session (${holder.slice(0, 8)}…). Wait for that session to finish or run /exit there.`;
}
    ...
    acquireCuLock: async () => {
      ...
      if (r.fresh) {
        ...
        tuc().sendOSNotification?.({
          message: escRegistered ? 'Rash is using your computer · press Esc to stop' : 'Rash is using your computer · press Ctrl+C to stop',
