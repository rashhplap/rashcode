 ...
 * rash-cli:// deep link. Linux xdg-open and browsers with "always allow"
 * set dispatch the link with no OS-level confirmation, so the application
 * provides its own provenance signal — mirroring rash.ai's security
 ...
 * notice which directory — and therefore which RASH.md — was loaded.
 */
 ...
 *
 * Always shows the working directory so the user can see which RASH.md
 ...
 * and whether its RASH.md may be stale relative to upstream.
 */
export function buildDeepLinkBanner(info: DeepLinkBannerInfo): string {
  ...
  if (info.repo) {
    ...
    lines.push(
      `Resolved ${info.repo} from local clones · last fetched ${age}${stale ? ' — RASH.md may be stale' : ''}`,
    )
  }
