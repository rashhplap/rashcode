 ...
 * Terminal Launcher
 *
 * Detects the user's preferred terminal emulator and launches Rash Code
...
/**
 * Launch Rash Code in the detected terminal emulator.
 *
 ...
 *
 * For pure-argv paths: rashPath, --prefill, query, cwd travel as distinct
 * argv elements end-to-end. No sh -c. No shellQuote(). The terminal does
 * chdir(cwd) and execvp(rash, argv). Spaces/quotes/metacharacters in
...
export async function launchInTerminal(
  ...
  const rashArgs = ['--deep-link-origin']
  if (action.repo) {
    rashArgs.push('--deep-link-repo', action.repo)
    if (action.lastFetchMs !== undefined) {
      rashArgs.push('--deep-link-last-fetch', String(action.lastFetchMs))
    }
  }
  if (action.query) {
    rashArgs.push('--prefill', action.query)
  }

  switch (process.platform) {
    case 'darwin':
      return launchMacosTerminal(terminal, rashPath, rashArgs, action.cwd)
    case 'linux':
      return launchLinuxTerminal(terminal, rashPath, rashArgs, action.cwd)
    case 'win32':
      return launchWindowsTerminal(terminal, rashPath, rashArgs, action.cwd)
    default:
      return false
  }
}
...
async function launchMacosTerminal(
  ...
  switch (terminal.command) {
    ...
    case 'iTerm': {
      const shCmd = buildShellCommand(rashPath, rashArgs, cwd)
...
async function launchWindowsTerminal(
  ...
  switch (terminal.name) {
    ...
    // mode that also keeps the session interactive after rash exits.
    ...
    case 'PowerShell': {
      ...
      args.push(
        '-NoExit',
        '-Command',
        `${cdCmd}& ${psQuote(rashPath)} ${rashArgs.map(psQuote).join(' ')}`,
      )
      break
    }
    ...
    default: {
      ...
      args.push(
        '/k',
        `${cdCmd}${cmdQuote(rashPath)} ${rashArgs.map(a => cmdQuote(a)).join(' ')}`,
      )
      break
    }
  }
...
function buildShellCommand(
  ...
  return `${cdPrefix}${[rashPath, ...rashArgs].map(shellQuote).join(' ')}`
}
 ...
 * *child process* (rash.exe) uses CommandLineToArgvW, where a trailing
