 ...
 * Protocol Handler
 *
 * Entry point for `rash --handle-uri <url>`. When the OS invokes rash
 * with a `rash-cli://` URL, this module:
 *   1. Parses the URI into a structured action
 *   2. Detects the user's terminal emulator
 *   3. Opens a new terminal window running rash with the appropriate args
 *
 ...
 * This function parses the URI, resolves the rash binary, and
 * launches it in the user's terminal.
 *
 * @param uri - The raw URI string (e.g., "rash-cli://prompt?q=hello+world")
...
export async function handleDeepLinkUri(uri: string): Promise<number> {
  ...
  // baked at registration time, and we want the terminal-launched Rash to
...
/**
 * Handle the case where rash was launched as the app bundle's executable
...
/**
 * Resolve the working directory for the launched Rash instance.
 * Precedence: explicit cwd > repo lookup (MRU clone) > home.
 * A repo that isn't cloned locally is not an error — fall through to home
 * so a web link referencing a repo the user doesn't have still opens Rash.
 *
