...
/**
 * Convert a PersistedStatsCache to RashCodeStats by computing derived fields.
 */
...
/**
 * Aggregates stats from all Rash Code sessions across all projects.
 * Uses a disk cache to avoid reprocessing historical data.
 */
export async function aggregateRashCodeStats(): Promise<RashCodeStats> {
 ...
 */
export async function aggregateRashCodeStatsForRange(
  range: StatsDateRange,
): Promise<RashCodeStats> {
  if (range === 'all') {
    return aggregateRashCodeStats()
  }
  ...
  return processedStatsToRashCodeStats(stats)
}

/**
 * Convert ProcessedStats to RashCodeStats.
 * Used for filtered date ranges that bypass the cache.
 */
function processedStatsToRashCodeStats(
...
function getEmptyStats(): RashCodeStats {
  return {
    totalSessions: 0,
    totalMessages: 0,
    totalDays: 0,
