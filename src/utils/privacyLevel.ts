 ...
 *   RASH_CODE_DISABLE_NONESSENTIAL_TRAFFIC  →  essential-traffic
...
export function getPrivacyLevel(): PrivacyLevel {
  if (process.env.RASH_CODE_DISABLE_NONESSENTIAL_TRAFFIC) {
 ...
 * Equivalent to the old `process.env.RASH_CODE_DISABLE_NONESSENTIAL_TRAFFIC` check.
 */
...
export function getEssentialTrafficOnlyReason(): string | null {
  if (process.env.RASH_CODE_DISABLE_NONESSENTIAL_TRAFFIC) {
    return 'RASH_CODE_DISABLE_NONESSENTIAL_TRAFFIC'
  }
  return null
}
