...
// Rash configuration directory names
export const RASH_CONFIG_DIRECTORIES = [
  'commands',
  'agents',
  'output-styles',
  'skills',
...
export type RashConfigDirectory = (typeof RASH_CONFIG_DIRECTORIES)[number]
 ...
 * See: https://github.com/anthropics/rash-code/issues/13893
 *
 ...
 * stopping there makes the parent project's `.rash/` unreachable (#31905).
 *
 ...
 *
 * Worktrees (under `.rash/worktrees/`) stay on the old behavior: their `.git`
 ...
 * collecting all .rash directories along the way.
 *
 * Stopping at git root prevents commands/skills from parent directories outside the repository
 * from leaking into projects. For example, if ~/projects/.rash/commands/ exists, it won't
 ...
 * @returns Array of directory paths containing .rash/subdir, from most specific (cwd) to least specific
 */
export function getProjectDirsUpToHome(
  subdir: RashConfigDirectory,
  cwd: string,
): string[] {
  ...
  while (true) {
    ...
    const rashSubdir = join(current, '.rash', subdir)
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    const userDir = join(getRashConfigHomeDir(), subdir)
    const managedDir = join(getManagedFilePath(), '.rash', subdir)
    const projectDirs = getProjectDirsUpToHome(subdir, cwd)

    // For git worktrees where the worktree does NOT have .rash/<subdir> checked
    ...
    //
    // Only add the main repo's copy when the worktree root's .rash/<subdir>
    // is absent. A standard `git worktree add` checks out the full tree, so the
    // worktree already has identical .rash/<subdir> content — loading the main
    // repo's copy too would duplicate every command/agent/skill
    // (anthropics/rash-code#29599, #28182, #26992).
    //
    ...
    if (gitRoot && canonicalRoot && canonicalRoot !== gitRoot) {
      const worktreeSubdir = normalizePathForComparison(
        join(gitRoot, '.rash', subdir),
      )
      const worktreeHasSubdir = projectDirs.some(
        dir => normalizePathForComparison(dir) === worktreeSubdir,
      )
      if (!worktreeHasSubdir) {
        const mainRashSubdir = join(canonicalRoot, '.rash', subdir)
        if (!projectDirs.includes(mainRashSubdir)) {
          projectDirs.push(mainRashSubdir)
        }
      }
    }
    ...
    // This prevents the same file from appearing multiple times when ~/.rash is
  ...
  (subdir: RashConfigDirectory, cwd: string) => `${subdir}:${cwd}`,
)
 ...
 * 3. Can be explicitly enabled via RASH_CODE_USE_NATIVE_FILE_SEARCH env var
 *
 * Symlink handling:
 ...
 * @param dir Directory (eg. "~/.rash/commands")
 * @returns Array of parsed markdown files with metadata
 */
...
> {
  ...
  // - Fallback: native Node.js (when RASH_CODE_USE_NATIVE_FILE_SEARCH is set)
  //
  // Why both? Ripgrep has poor startup performance in native builds.
  const useNative = isEnvTruthy(process.env.RASH_CODE_USE_NATIVE_FILE_SEARCH)
