...
import {
  RASH_FOLDER_PERMISSION_PATTERN,
  FILE_EDIT_TOOL_NAME,
  GLOBAL_RASH_FOLDER_PERMISSION_PATTERN,
 ...
 * filesystems (macOS/Windows) like `.rAsH/Settings.locaL.json`.
 *
...
/**
 * If filePath is inside a .rash/skills/{name}/ directory (project or global),
 ...
 * require granting session access to all of .rash/ (settings.json, hooks/, etc.).
 */
export function getRashSkillScope(
  filePath: string,
  ...
  const bases = [
    {
      dir: expandPath(join(getOriginalCwd(), '.rash', 'skills')),
      prefix: '/.rash/skills/',
    },
    {
      dir: expandPath(join(homedir(), '.rash', 'skills')),
      prefix: '~/.rash/skills/',
    },
  ]
  ...
  for (const { dir, prefix } of bases) {
    ...
    for (const s of [sep, '/']) {
      if (absolutePathLower.startsWith(dirLower + s.toLowerCase())) {
        ...
        // produce '/.rash/skills/*/**' which matches ALL skills. Return null
...
export function isRashSettingsPath(filePath: string): boolean {
  // SECURITY: Normalize path structure first to prevent bypass via redundant ./
  // sequences like `./.rash/./settings.json` which would evade the endsWith() check
  const expandedPath = expandPath(filePath)

  // Normalize for case-insensitive comparison to prevent bypassing security
  // with paths like .rAsH/Settings.locaL.json
  const normalizedPath = normalizeCaseForComparison(expandedPath)

  // Use platform separator so endsWith checks work on both Unix (/) and Windows (\)
  if (
    normalizedPath.endsWith(`${sep}.rash${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.rash${sep}settings.local.json`)
  ) {
    // Include .rash/settings.json even for other projects
    return true
  }
...
// Always ask when Rash Code tries to edit its own config files
function isRashConfigFilePath(filePath: string): boolean {
  if (isRashSettingsPath(filePath)) {
    return true
  }

  // Check if file is within .rash/commands or .rash/agents directories
  // using proper path segment validation (not string matching with includes())
  // pathInWorkingPath now handles case-insensitive comparison to prevent bypasses
  const commandsDir = join(getOriginalCwd(), '.rash', 'commands')
  const agentsDir = join(getOriginalCwd(), '.rash', 'agents')
  const skillsDir = join(getOriginalCwd(), '.rash', 'skills')
 ...
 * Path format: ~/.rash/projects/{sanitized-cwd}/...
 */
 ...
 * The scratchpad is a per-session directory for Rash to write temporary files.
...
/**
 * Returns the user-specific Rash temp directory name.
 * On Unix: 'rash-{uid}' to prevent multi-user permission conflicts
 * On Windows: 'rash' (tmpdir() is already per-user)
 */
export function getRashTempDirName(): string {
  if (getPlatform() === 'windows') {
    return 'rash'
  }
...
/**
 * Returns the Rash temp directory path with symlinks resolved.
 * Uses TMPDIR env var if set, otherwise:
 * - On Unix: /tmp/rash-{uid}/ (resolved to /private/tmp/rash-{uid}/ on macOS)
 * - On Windows: {tmpdir}/rash/ (e.g., C:\Users\{user}\AppData\Local\Temp\rash\)
 * This is a per-user temporary directory used by Rash Code for all temp files.
 *
 ...
 * resolution, paths like /tmp/rash-{uid}/... wouldn't match /private/tmp/rash-{uid}/...
 */
// Memoized: called per-tool from permission checks (yoloClassifier, sandbox-adapter)
// and per-turn from BashTool prompt. Inputs (RASH_CODE_TMPDIR env + platform) are
// fixed at startup, and the realpath of the system tmp dir does not change mid-session.
export const getRashTempDir = memoize(function getRashTempDir(): string {
  const baseTmpDir =
    process.env.RASH_CODE_TMPDIR ||
  ...
  return join(resolvedBaseTmpDir, getRashTempDirName()) + sep
})
...
export const getBundledSkillsRoot = memoize(
  function getBundledSkillsRoot(): string {
    ...
    return join(getRashTempDir(), 'bundled-skills', MACRO.VERSION, nonce)
  },
)

/**
 * Returns the project temp directory path with trailing separator.
 * Path format: /tmp/rash-{uid}/{sanitized-cwd}/
 */
export function getProjectTempDir(): string {
  return join(getRashTempDir(), sanitizePath(getOriginalCwd())) + sep
}

/**
 * Returns the scratchpad directory path for the current session.
 * Path format: /tmp/rash-{uid}/{sanitized-cwd}/{sessionId}/scratchpad/
 */
...
function isScratchpadPath(absolutePath: string): boolean {
  ...
  //   echo "malicious" > /tmp/rash-0/proj/session/scratchpad/../../../etc/passwd
...
function isDangerousFilePathToAutoEdit(path: string): boolean {
  ...
  for (let i = 0; i < pathSegments.length; i++) {
    ...
    for (const dir of DANGEROUS_DIRECTORIES) {
      ...
      // Special case: .rash/worktrees/ is a structural path (where Rash stores
      // git worktrees), not a user-created dangerous directory. Skip the .rash
      // segment when it's followed by 'worktrees'. Any nested .rash directories
      ...
      if (dir === '.rash') {
        ...
        if (
          ...
          break // Skip this .rash, continue checking other segments
        }
      }
 ...
 * - 8.3 short names (e.g., GIT~1, RASH~1, SETTIN~1.JSON)
 * - Long path prefixes (e.g., \\?\C:\..., \\.\C:\..., //?/C:/..., //./C:/...)
 * - Trailing dots and spaces (e.g., .git., .rash , .bashrc...)
...
function hasSuspiciousWindowsPathPattern(path: string): boolean {
  ...
  // Examples: GIT~1, RASH~1, SETTIN~1.JSON, BASHRC~1
  if (/~\d/.test(path)) {
    return true
  }
  ...
  // Examples: .git., .rash , .bashrc..., settings.json.
 ...
 * - Rash config files (.rash/settings.json, .rash/commands/, .rash/agents/)
 * - MCP CLI state files (managed internally by Rash Code)
...
export function checkPathSafetyForAutoEdit(
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        safe: false,
        message: `Rash requested permissions to write to ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        classifierApprovable: false,
      }
    }
  }

  // Check for Rash config files on all paths
  for (const pathToCheck of pathsToCheck) {
    if (isRashConfigFilePath(pathToCheck)) {
      return {
        safe: false,
        message: `Rash requested permissions to write to ${path}, but you haven't granted it yet.`,
  ...
  for (const pathToCheck of pathsToCheck) {
    if (isDangerousFilePathToAutoEdit(pathToCheck)) {
      return {
        safe: false,
        message: `Rash requested permissions to edit ${path} which is a sensitive file.`,
        classifierApprovable: true,
      }
    }
  }
...
export function pathInWorkingPath(path: string, workingPath: string): boolean {
  ...
  // checks on case-insensitive filesystems (macOS/Windows) like .rAsH/CoMmAnDs
...
function patternWithRoot(
  ...
  if (pattern.startsWith(`${DIR_SEP}${DIR_SEP}`)) {
    ...
    // Patterns starting with / resolve relative to the directory where settings are stored (without .rash/)
    return {
...
export function checkReadPermissionForTool(
  ...
  if (typeof tool.getPath !== 'function') {
    return {
      behavior: 'ask',
      message: `Rash requested permissions to use ${tool.name}, but you haven't granted it yet.`,
    }
  }
  ...
  for (const pathToCheck of pathsToCheck) {
    if (pathToCheck.startsWith('\\\\') || pathToCheck.startsWith('//')) {
      return {
        behavior: 'ask',
        message: `Rash requested permissions to read from ${path}, which appears to be a UNC path that could access network resources.`,
        decisionReason: {
          type: 'other',
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        behavior: 'ask',
        message: `Rash requested permissions to read from ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        decisionReason: {
          type: 'other',
          reason:
  ...
  for (const pathToCheck of pathsToCheck) {
    ...
    if (askRule) {
      return {
        behavior: 'ask',
        message: `Rash requested permissions to read from ${path}, but you haven't granted it yet.`,
        decisionReason: {
          type: 'rule',
          rule: askRule,
        },
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  // This MUST come before isDangerousFilePathToAutoEdit check since .rash is a dangerous directory
  ...
  // 1.6. Check for .rash/** allow rules BEFORE safety checks
  // This allows session-level permissions to bypass the safety blocks for .rash/
  // We only allow this for session-level rules to prevent users from accidentally
  // permanently granting broad access to their .rash/ folder.
  //
  // matchingRuleForInput returns the first match across all sources. If the user
  // also has a broader Edit(.rash) rule in userSettings (e.g. from sandbox
  ...
  // "allow Rash to edit its own settings for this session" option actually works.
  const rashFolderAllowRule = matchingRuleForInput(
    path,
    {
      ...toolPermissionContext,
      alwaysAllowRules: {
  ...
  if (rashFolderAllowRule) {
    // Check if this rule is scoped under .rash/ (project or global).
    // Accepts both the broad patterns ('/.rash/**', '~/.rash/**') and
    // narrowed ones like '/.rash/skills/my-skill/**' so users can grant
    ...
    // '/.rash/../**' from leaking this bypass outside .rash/.
    const ruleContent = rashFolderAllowRule.ruleValue.ruleContent
    if (
      ruleContent &&
      (ruleContent.startsWith(RASH_FOLDER_PERMISSION_PATTERN.slice(0, -2)) ||
        ruleContent.startsWith(
          GLOBAL_RASH_FOLDER_PERMISSION_PATTERN.slice(0, -2),
        )) &&
      ...
      return {
        ...
        decisionReason: {
          type: 'rule',
          rule: rashFolderAllowRule,
        },
      }
    }
  }

  // 1.7. Check comprehensive safety validations (Windows patterns, Rash config, dangerous files)
  ...
  if (!safetyCheck.safe) {
    // SDK suggestion: if under .rash/skills/{name}/, emit the narrowed
    // session-scoped addRules that step 1.6 will honor on the next call.
    // Everything else (.rash/settings.json, .git/, .vscode/, .idea/) falls
    ...
    const skillScope = getRashSkillScope(path)
...
export function checkEditableInternalPath(
  ...
  // var value must itself resolve under ~/.rash/jobs/. Symlink guard: every
  ...
  if (feature('TEMPLATES')) {
    const jobDir = process.env.RASH_JOB_DIR
    if (jobDir) {
      const jobsRoot = join(getRashConfigHomeDir(), 'jobs')
      ...
      // the case where ~/.rash is a symlink (e.g. to /data/rash-config).
  ...
  // ~/.rash/, which is in DANGEROUS_DIRECTORIES. The RASH_COWORK_MEMORY_PATH_OVERRIDE
  ...
  // .rash/launch.json — desktop preview config (dev server command + port).
  // The desktop's preview_start MCP tool instructs Rash to create/update
  // this file as part of the preview workflow. Without this carve-out the
  // .rash/ DANGEROUS_DIRECTORIES check prompts for it, which in SDK mode
  ...
  // .rash/ only (not ~/.rash/) since launch.json is per-project.
  if (
    normalizeCaseForComparison(normalizedPath) ===
    normalizeCaseForComparison(join(getOriginalCwd(), '.rash', 'launch.json'))
  ) {
    return {
      behavior: 'allow',
      updatedInput: input,
...
export function checkReadableInternalPath(
  ...
  // Project temp directory (/tmp/rash/{sanitized-cwd}/)
  ...
  // Tasks directory (~/.rash/tasks/) for swarm task coordination
  const tasksDir = join(getRashConfigHomeDir(), 'tasks') + sep
  if (
  ...
  // Teams directory (~/.rash/teams/) for swarm coordination
  const teamsReadDir = join(getRashConfigHomeDir(), 'teams') + sep
  if (
