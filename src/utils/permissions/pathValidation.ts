 ...
 * validation purposes, so commands like `echo foo > /tmp/rash/x.txt` don't
 * prompt for permission when /tmp/rash/ is already in the sandbox allowlist.
 *
 * Respects the deny-within-allow list: paths in denyWithinAllow (like
 * .rash/settings.json) are still blocked even if their parent is in allowOnly.
 */
...
export function isPathAllowed(
  ...
  // Checks: Windows patterns, Rash config files, dangerous files (on original + symlink paths)
  if (operationType !== 'read') {
