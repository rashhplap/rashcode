...
async function checkRashMdFiles(): Promise<ContextWarning | null> {
  ...
  const message =
    largeFiles.length === 1
      ? `Large RASH.md file detected (${largeFiles[0]!.content.length.toLocaleString()} chars > ${MAX_MEMORY_CHARACTER_COUNT.toLocaleString()})`
      : `${largeFiles.length} large RASH.md files detected (each > ${MAX_MEMORY_CHARACTER_COUNT.toLocaleString()} chars)`
...
export async function checkContextWarnings(
  ...
  const [rashMdWarning, agentWarning, mcpWarning, unreachableRulesWarning] =
    await Promise.all([
      checkRashMdFiles(),
