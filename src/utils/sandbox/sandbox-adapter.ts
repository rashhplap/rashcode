/**
 * Adapter layer that wraps @anthropic-ai/sandbox-runtime with Rash CLI-specific integrations.
 * This file provides the bridge between the external sandbox-runtime package and Rash CLI's
...
import { getRashTempDir } from '../permissions/filesystem.js'
...
/**
 * Resolve Rash Code-specific path patterns for sandbox-runtime.
 *
 * Rash Code uses special path prefixes in permission rules:
...
/**
 * Convert Rash Code settings format to SandboxRuntimeConfig format
...
export function convertToSandboxRuntimeConfig(
  ...
  // Always include current directory and Rash temp directory as writable
  // The temp directory is needed for Shell.ts cwd tracking files
  const allowWrite: string[] = ['.', getRashTempDir()]
  const denyWrite: string[] = []
  const denyRead: string[] = []
  const allowRead: string[] = []

  // Always deny writes to settings.json files to prevent sandbox escape
  // This blocks settings in the original working directory (where Rash Code started)
  ...
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.rash', 'settings.json'))
    denyWrite.push(resolve(cwd, '.rash', 'settings.local.json'))
  }

  // Block writes to .rash/skills in both original and current working directories.
  // The sandbox-runtime's getDangerousDirectories() protects .rash/commands and
  // .rash/agents but not .rash/skills. Skills have the same privilege level
  // (auto-discovered, auto-loaded, full Rash capabilities) so they need the
  // same OS-level sandbox protection.
  denyWrite.push(resolve(originalCwd, '.rash', 'skills'))
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.rash', 'skills'))
  }

  // SECURITY: Git's is_git_directory() treats cwd as a bare repo if it has
  // HEAD + objects/ + refs/. An attacker planting these (plus a config with
  // core.fsmonitor) escapes the sandbox when Rash's unsandboxed git runs.
  //
...
// scrubbed if they appear after a sandboxed command. See anthropics/rash-code#29316.
const bareGitRepoScrubPaths: string[] = []

/**
 * Delete bare-repo files planted at cwd during a sandboxed command, before
 * Rash's unsandboxed git calls can see them. See the SECURITY block above
 * bareGitRepoFiles. anthropics/rash-code#29316.
 */
 ...
 * This is a Rash CLI-specific function that updates local settings.
 */
...
/**
 * Rash CLI sandbox manager - wraps sandbox-runtime with Rash-specific features
 */
