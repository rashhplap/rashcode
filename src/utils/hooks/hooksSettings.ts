...
export function getAllHooks(appState: AppState): IndividualHookConfig[] {
  ...
  if (!restrictedToManagedOnly) {
    ...
    // both resolve to ~/.rash/settings.json)
...
export function hookSourceDescriptionDisplayString(source: HookSource): string {
  switch (source) {
    case 'userSettings':
      return 'User settings (~/.rash/settings.json)'
    case 'projectSettings':
      return 'Project settings (.rash/settings.json)'
    case 'localSettings':
      return 'Local settings (.rash/settings.local.json)'
    case 'pluginHook':
      ...
      // e.g., "Plugin hooks (~/.rash/plugins/repos/source/example-plugin/example-plugin/hooks/hooks.json)"
      return 'Plugin hooks (~/.rash/plugins/*/hooks/hooks.json)'
    case 'sessionHook':
      return 'Session hooks (in-memory, temporary)'
    case 'builtinHook':
      return 'Built-in hooks (registered internally by Rash Code)'
    default:
