 ...
 *
 * The parent rash process keeps these vars (needed for API calls, lazy
...
const GHA_SUBPROCESS_SCRUB = [
  // Anthropic auth — rash re-reads these per-request, subprocesses don't need them
  'ANTHROPIC_API_KEY',
  ...
  // GitHub Actions OIDC — consumed by the action's JS before rash spawns;
  ...
  // rash-code-action-specific duplicates — action JS consumes these during
  // prepare, before spawning rash. ALL_INPUTS contains anthropic_api_key as JSON.
  'ALL_INPUTS',
 ...
 * servers, shell hooks).
 *
 * Gated on RASH_CODE_SUBPROCESS_ENV_SCRUB. rash-code-action sets this
...
export function subprocessEnv(): NodeJS.ProcessEnv {
  ...
  if (!isEnvTruthy(process.env.RASH_CODE_SUBPROCESS_ENV_SCRUB)) {
