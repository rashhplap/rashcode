...
export const getGlobalRashFile = memoize((): string => {
  // Legacy fallback for backwards compatibility
  if (
    getFsImplementation().existsSync(
      join(getRashConfigHomeDir(), '.config.json'),
    )
  ) {
    return join(getRashConfigHomeDir(), '.config.json')
  }

  const filename = `.rash${fileSuffixForOauthConfig()}.json`
  return join(process.env.RASH_CONFIG_DIR || homedir(), filename)
})
 ...
 * If RASH_CODE_HOST_PLATFORM is set to a valid platform value, that overrides
...
export function getHostPlatformForAnalytics(): Platform {
  const override = process.env.RASH_CODE_HOST_PLATFORM
