 ...
 * user-controlled files (~/.rash/settings.json and ~/.rash.json),
 ...
 *
 * We read from global config (~/.rash.json) and user settings
 * (~/.rash/settings.json). These are user-controlled files that don't
...
function getExtraCertsPathFromConfig(): string | undefined {
  try {
    ...
    // Only read from user-controlled settings (~/.rash/settings.json),
