...
/**
 * `rash ssh` remote: ANTHROPIC_UNIX_SOCKET routes auth through a -R forwarded
 * socket to a local proxy, and the launcher sets a handful of placeholder auth
 * env vars that the remote's ~/.rash settings.env MUST NOT clobber (see
 ...
 * RASH_CODE_PROVIDER_MANAGED_BY_HOST in spawn env), strip
 * provider-selection / model-default vars from settings-sourced env so a
 * user's ~/.rash/settings.json can't redirect requests away from the
...
function withoutHostManagedProviderVars(
  ...
  if (!env) return {}
  if (!isEnvTruthy(process.env.RASH_CODE_PROVIDER_MANAGED_BY_HOST)) {
    return env
  }
 ...
 *
 * - userSettings (~/.rash/settings.json): controlled by the user, not project-specific
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // Global config (~/.rash.json) is user-controlled. In CCD mode,
  ...
  // doesn't get clobbered by ~/.rash/settings.json env (gh#217). policy/flag
  ...
  // flagSettings env applied. Eligibility reads RASH_CODE_USE_BEDROCK,
  ...
  // when RASH_CODE_PROVIDER_MANAGED_BY_HOST is set.
 ...
 * RASH_CODE_PROVIDER_MANAGED_BY_HOST is set — see filterSettingsEnv) and
