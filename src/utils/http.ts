...
import { getRashCodeUserAgent } from './userAgent.js'
import { getWorkload } from './workloadContext.js'

// WARNING: We rely on `rash-cli` in the user agent for log filtering.
// Please do NOT change this without making sure that logging also gets updated!
export function getUserAgent(): string {
  const agentSdkVersion = process.env.RASH_AGENT_SDK_VERSION
    ? `, agent-sdk/${process.env.RASH_AGENT_SDK_VERSION}`
    : ''
  // SDK consumers can identify their app/library via RASH_AGENT_SDK_CLIENT_APP
  // e.g., "my-app/1.0.0" or "my-library/2.1"
  const clientApp = process.env.RASH_AGENT_SDK_CLIENT_APP
    ? `, client-app/${process.env.RASH_AGENT_SDK_CLIENT_APP}`
    : ''
  ...
  return `rash-cli/${MACRO.VERSION} (${process.env.USER_TYPE}, ${process.env.RASH_CODE_ENTRYPOINT ?? 'cli'}${agentSdkVersion}${clientApp}${workloadSuffix})`
}

export function getMCPUserAgent(): string {
  const parts: string[] = []
  if (process.env.RASH_CODE_ENTRYPOINT) {
    parts.push(process.env.RASH_CODE_ENTRYPOINT)
  }
  if (process.env.RASH_AGENT_SDK_VERSION) {
    parts.push(`agent-sdk/${process.env.RASH_AGENT_SDK_VERSION}`)
  }
  if (process.env.RASH_AGENT_SDK_CLIENT_APP) {
    parts.push(`client-app/${process.env.RASH_AGENT_SDK_CLIENT_APP}`)
  }
  const suffix = parts.length > 0 ? ` (${parts.join(', ')})` : ''
  return `rash-code/${MACRO.VERSION}${suffix}`
}

// User-Agent for WebFetch requests to arbitrary sites. `Rash-User` is
// Anthropic's publicly documented agent for user-initiated fetches (what site
// operators match in robots.txt); the rash-code suffix lets them distinguish
// local CLI traffic from rash.ai server-side fetches.
export function getWebFetchUserAgent(): string {
  return `Rash-User (${getRashCodeUserAgent()}; +https://support.anthropic.com/)`
}
...
export async function withOAuth401Retry<T>(
  ...
  try {
    ...
    const failedAccessToken = getRashAIOAuthTokens()?.accessToken
