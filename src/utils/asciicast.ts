 ...
 * For ants with RASH_CODE_TERMINAL_RECORDING=1: returns a path.
...
export function getRecordFilePath(): string | null {
  ...
  if (!isEnvTruthy(process.env.RASH_CODE_TERMINAL_RECORDING)) {
    return null
  }
  // Record alongside the transcript.
  // Each launch gets its own file so --continue produces multiple recordings.
  const projectsDir = join(getRashConfigHomeDir(), 'projects')
