 ...
 * - Checks environment kind (RASH_CODE_ENVIRONMENT_KIND)
 * - Retrieves session access token
 * - Requires RASH_CODE_REMOTE_SESSION_ID for session ID
 *
...
export async function runFilePersistence(
  ...
  const sessionId = process.env.RASH_CODE_REMOTE_SESSION_ID
  if (!sessionId) {
    logError(
      new Error(
        'File persistence enabled but RASH_CODE_REMOTE_SESSION_ID is not set',
      ),
    )
    return null
  }
