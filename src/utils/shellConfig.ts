 ...
 * Used for managing rash aliases and PATH entries
 */
...
import { getLocalRashPath } from './localInstaller.js'

export const RASH_ALIAS_REGEX = /^\s*alias\s+rash\s*=/
...
/**
 * Filter out installer-created rash aliases from an array of lines
 * Only removes aliases pointing to $HOME/.rash/local/rash
 ...
 */
export function filterRashAliases(lines: string[]): {
  filtered: string[]
  hadAlias: boolean
} {
  let hadAlias = false
  const filtered = lines.filter(line => {
    // Check if this is a rash alias
    if (RASH_ALIAS_REGEX.test(line)) {
      // Extract the alias target - handle spaces, quotes, and various formats
      // First try with quotes
      let match = line.match(/alias\s+rash\s*=\s*["']([^"']+)["']/)
      if (!match) {
        // Try without quotes (capturing until end of line or comment)
        match = line.match(/alias\s+rash\s*=\s*([^#\n]+)/)
      }
...
/**
 * Check if a rash alias exists in any shell config file
 * Returns the alias target if found, null otherwise
 * @param options Optional overrides for testing (env, homedir)
 */
export async function findRashAlias(
  options?: ShellConfigOptions,
): Promise<string | null> {
  ...
  for (const configPath of Object.values(configs)) {
    ...
    for (const line of lines) {
      if (RASH_ALIAS_REGEX.test(line)) {
        ...
        const match = line.match(/alias\s+rash=["']?([^"'\s]+)/)
        if (match && match[1]) {
          return match[1]
        }
      }
...
/**
 * Check if a rash alias exists and points to a valid executable
 * Returns the alias target if valid, null otherwise
 * @param options Optional overrides for testing (env, homedir)
 */
export async function findValidRashAlias(
  options?: ShellConfigOptions,
): Promise<string | null> {
  const aliasTarget = await findRashAlias(options)
