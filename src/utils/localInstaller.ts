...
// hfi.tsx get a chance to set RASH_CONFIG_DIR in main(), and would also
// populate the memoize cache with that stale value for all 150+ other callers.
function getLocalInstallDir(): string {
  return join(getRashConfigHomeDir(), 'local')
}
export function getLocalRashPath(): string {
  return join(getLocalInstallDir(), 'rash')
}
...
export function isRunningFromLocalInstallation(): boolean {
  ...
  return execPath.includes('/.rash/local/node_modules/')
}
...
export async function ensureLocalPackageEnvironment(): Promise<boolean> {
  try {
    ...
    await writeIfMissing(
      ...
      jsonStringify(
        { name: 'rash-local', version: '0.0.1', private: true },
        null,
        2,
      ),
    )
    ...
    const wrapperPath = join(localInstallDir, 'rash')
    const created = await writeIfMissing(
      wrapperPath,
      `#!/bin/sh\nexec "${localInstallDir}/node_modules/.bin/rash" "$@"`,
      0o755,
    )
    if (created) {
...
/**
 * Install or update Rash CLI package in the local directory
 ...
 */
export async function installOrUpdateRashPackage(
  ...
  try {
    ...
    if (result.code !== 0) {
      const error = new Error(
        `Failed to install Rash CLI package: ${result.stderr}`,
      )
      logError(error)
...
export async function localInstallationExists(): Promise<boolean> {
  try {
    await access(join(getLocalInstallDir(), 'node_modules', '.bin', 'rash'))
    return true
  } catch {
    return false
  }
