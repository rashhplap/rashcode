...
function detectShell(): ShellInfo | null {
  ...
  const home = homedir()
  const rashDir = join(home, '.rash')

  if (shell.endsWith('/zsh') || shell.endsWith('/zsh.exe')) {
    const cacheFile = join(rashDir, 'completion.zsh')
    return {
      name: 'zsh',
  ...
  if (shell.endsWith('/bash') || shell.endsWith('/bash.exe')) {
    const cacheFile = join(rashDir, 'completion.bash')
    return {
      name: 'bash',
  ...
  if (shell.endsWith('/fish') || shell.endsWith('/fish.exe')) {
    ...
    const cacheFile = join(rashDir, 'completion.fish')
    return {
      name: 'fish',
...
export async function setupShellCompletion(theme: ThemeName): Promise<string> {
  ...
  try {
    ...
    logError(e)
    return `${EOL}${color('warning', theme)(`Could not write ${shell.name} completion cache`)}${EOL}${chalk.dim(`Run manually: rash completion ${shell.shellFlag} > ${shell.cacheFile}`)}${EOL}`
  }
  ...
  const rashBin = process.argv[1] || 'rash'
  const result = await execFileNoThrow(rashBin, [
    'completion',
    shell.shellFlag,
    '--output',
    shell.cacheFile,
  ...
  if (result.code !== 0) {
    return `${EOL}${color('warning', theme)(`Could not generate ${shell.name} shell completions`)}${EOL}${chalk.dim(`Run manually: rash completion ${shell.shellFlag} > ${shell.cacheFile}`)}${EOL}`
  }
  ...
  try {
    ...
    if (
      existing.includes('rash completion') ||
...
/**
 * Regenerate cached shell completion scripts in ~/.rash/.
 * Called after `rash update` so completions stay in sync with the new binary.
 */
