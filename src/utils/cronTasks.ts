// Scheduled prompts, stored in <project>/.rash/scheduled_tasks.json.
//
...
const CRON_FILE_REL = join('.rash', 'scheduled_tasks.json')
...
/**
 * Read and parse .rash/scheduled_tasks.json. Returns an empty task list if the file
...
/**
 * Overwrite .rash/scheduled_tasks.json with the given tasks. Creates .rash/ if
...
export async function writeCronTasks(
  ...
  await mkdir(join(root, '.rash'), { recursive: true })
 ...
 * written to .rash/scheduled_tasks.json and dies with the process. The
 ...
 * and recurring tasks — a recurring task whose window passed while Rash
