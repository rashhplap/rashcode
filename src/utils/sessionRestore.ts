...
import { clearMemoryFileCaches } from './rashmd.js'
import {
 ...
 * Used for computing initial state before render (per RASH.md guidelines).
...
export async function processResumedConversation(
  ...
  if (!opts.forkSession) {
    ...
    // the pre-loaded records). Without this seed, `rash -r {newSessionId}`
  ...
  // Compute initial state before render (per RASH.md guidelines)
