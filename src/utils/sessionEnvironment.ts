...
export async function getSessionEnvironmentScript(): Promise<string | null> {
  ...
  // Check for RASH_ENV_FILE passed from parent process (e.g., HFI trajectory runner)
  // This allows venv/conda activation to persist across shell commands
  const envFile = process.env.RASH_ENV_FILE
  if (envFile) {
    try {
      ...
      if (envScript) {
        ...
        logForDebugging(
          `Session environment loaded from RASH_ENV_FILE: ${envFile} (${envScript.length} chars)`,
        )
      }
    } catch (e: unknown) {
      const code = getErrnoCode(e)
      if (code !== 'ENOENT') {
        logForDebugging(`Failed to read RASH_ENV_FILE: ${errorMessage(e)}`)
      }
    }
  }
