...
const resolveCanonicalRoot = memoizeWithLRU(
  (gitRoot: string): string => {
    try {
      ...
      // dialog and executing hooks from .rash/settings.json on startup.
      //
      ...
      // Use the common dir itself as the stable identity (anthropics/rash-code#27994).
...
export const stashToCleanState = async (message?: string): Promise<boolean> => {
  try {
    const stashMessage =
      message || `Rash Code auto-stash - ${new Date().toISOString()}`
