...
// imports above. Named (not wildcard) per RASH.md style; no collisions
...
export function getProjectsDir(): string {
  return join(getRashConfigHomeDir(), 'projects')
}
...
function getEntrypoint(): string | undefined {
  return process.env.RASH_CODE_ENTRYPOINT
}
 ...
 * This ensures tests with different RASH_CONFIG_DIR values
...
class Project {
   ...
   * RASH_CODE_SKIP_PROMPT_HISTORY should suppress all transcript writes.
  ...
  private shouldSkipPersistence(): boolean {
    ...
    return (
      ...
      isSessionPersistenceDisabled() ||
      isEnvTruthy(process.env.RASH_CODE_SKIP_PROMPT_HISTORY)
    )
  }
 ...
 *
 * Streaming (rash.ts:~2024) emits one AssistantMessage per content_block_stop
...
/**
 * Append a periodic task summary for `rash ps`. Unlike ai-title this is
...
export async function loadTranscriptFile(
  ...
  try {
    ...
    if (!isEnvTruthy(process.env.RASH_CODE_DISABLE_PRECOMPACT_SKIP)) {
    ...
    // RASH_CODE_DISABLE_PRECOMPACT_SKIP is set (that kill switch means
    ...
    if (
      !opts?.keepAllLeaves &&
      !hasPreservedSegment &&
      !isEnvTruthy(process.env.RASH_CODE_DISABLE_PRECOMPACT_SKIP) &&
...
export function isLoggableMessage(m: Message): boolean {
  ...
  if (m.type === 'attachment' && getUserType() !== 'ant') {
    if (
      ...
      isEnvTruthy(process.env.RASH_CODE_SAVE_HOOK_ADDITIONAL_CONTEXT)
    ) {
      return true
    }
    return false
