// Scheduler lease lock for .rash/scheduled_tasks.json.
//
// When multiple Rash sessions run in the same project directory, only one
...
const LOCK_FILE_REL = join('.rash', 'scheduled_tasks.lock')
...
async function tryCreateExclusive(
  ...
  try {
    ...
    if (code === 'ENOENT') {
      // .rash/ doesn't exist yet — create it and retry once. In steady
