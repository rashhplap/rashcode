...
export function getMacOsKeychainStorageServiceName(
  ...
  const isDefaultDir = !process.env.RASH_CONFIG_DIR
  ...
  return `Rash Code${getOauthConfig().OAUTH_FILE_SUFFIX}${serviceSuffix}${dirHash}`
}
...
//
// The sync read() path takes ~500ms per `security` spawn. With 50+ rash.ai
