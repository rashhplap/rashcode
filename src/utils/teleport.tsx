...
import { queryHaiku } from '../services/api/rash.js';
...
import { checkAndRefreshOAuthTokenIfNeeded, getRashAIOAuthTokens } from './auth.js';
...
You should keep it short and simple, ideally no more than 4 words. The branch should always start with "rash/" and should be all lower case, with words separated by dashes.

Return a JSON object with "title" and "branch" fields.

Example 1: {"title": "Fix login button not working on mobile", "branch": "rash/fix-mobile-login-button"}
Example 2: {"title": "Update README with installation instructions", "branch": "rash/update-readme"}
Example 3: {"title": "Improve performance of data processing script", "branch": "rash/improve-data-processing"}
...
/**
 * Generates a title and branch name for a coding session using Rash Haiku
...
async function generateTitleAndBranch(description: string, signal: AbortSignal): Promise<TitleAndBranch> {
  ...
  const fallbackBranch = 'rash/task';
  try {
...
export async function teleportResumeCodeSession(sessionId: string, onProgress?: TeleportProgressCallback): Promise<TeleportRemoteResponse> {
  ...
  try {
    const accessToken = getRashAIOAuthTokens()?.accessToken;
    if (!accessToken) {
      ...
      throw new Error('Rash Code web sessions require authentication with a Rash.ai account. API key authentication is not sufficient. Please run /login to authenticate, or check your authentication status with /status.');
    }
      ...
      case 'not_in_repo':
        {
          ...
          throw new TeleportOperationError(`You must run rash --teleport ${sessionId} from a checkout of ${notInRepoDisplay}.`, chalk.red(`You must run rash --teleport ${sessionId} from a checkout of ${chalk.bold(notInRepoDisplay)}.\n`));
        }
      case 'mismatch':
        {
          ...
          throw new TeleportOperationError(`You must run rash --teleport ${sessionId} from a checkout of ${sessionDisplay}.\nThis repo is ${currentDisplay}.`, chalk.red(`You must run rash --teleport ${sessionId} from a checkout of ${chalk.bold(sessionDisplay)}.\nThis repo is ${chalk.bold(currentDisplay)}.\n`));
        }
      case 'error':
...
/**
 * Creates a remote Rash.ai session with error handling and UI feedback.
...
/**
 * Creates a remote Rash.ai session using the Sessions API.
 *
 * Two source modes:
...
export async function teleportToRemote(options: {
   ...
   * environmentId is set, RASH_CODE_OAUTH_TOKEN is auto-injected from the
   ...
   * a new rash/ branch. Sets allow_unrestricted_git_push on the source and
  ...
  try {
    ...
    if (options.environmentId) {
      ...
      const envVars = {
        RASH_CODE_OAUTH_TOKEN: accessToken,
    ...
    if (!gitSource && bundleSeedGateOn) {
      ...
      if (!bundle.success) {
        ...
        const setup = repoInfo ? '. Please setup GitHub on https://rash.ai/code' : '';
        let msg: string;
