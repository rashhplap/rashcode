...
async function getInstallationPath(): Promise<string> {
  ...
  if (isInBundledMode()) {
    ...
    try {
      await getFsImplementation().stat(join(homedir(), '.local/bin/rash'))
      return join(homedir(), '.local/bin/rash')
    } catch {
      // Not found
    }
    return 'native'
...
> {
  ...
  const localPath = join(homedir(), '.rash', 'local')
  if (await localInstallationExists()) {
    installations.push({ type: 'npm-local', path: localPath })
  }

  // Check for global npm installation
  const packagesToCheck = ['@anthropic-ai/rash-code']
  ...
  if (npmResult.code === 0 && npmResult.stdout) {
    ...
    // First check for active installations via bin/rash
    // Linux / macOS have prefix/bin/rash and prefix/lib/node_modules
    // Windows has prefix/rash and prefix/node_modules
    const globalBinPath = isWindows
      ? join(npmPrefix, 'rash')
      : join(npmPrefix, 'bin', 'rash')
    ...
    if (globalBinExists) {
      ...
      // When npm is installed via Homebrew, both can exist at /opt/homebrew/bin/rash
      ...
      // If no bin/rash exists, check for orphaned packages (no bin/rash symlink)
  ...
  const nativeBinPath = join(homedir(), '.local', 'bin', 'rash')
  try {
  ...
  if (config.installMethod === 'native') {
    const nativeDataPath = join(homedir(), '.local', 'share', 'rash')
    try {
...
async function detectConfigurationIssues(
  ...
  if (!isEnvTruthy(process.env.DISABLE_INSTALLATION_CHECKS)) {
    if (type === 'npm-local' && config.installMethod !== 'local') {
      warnings.push({
        ...
        fix: 'Consider using native installation: rash install',
      })
    }
    ...
    if (type === 'native' && config.installMethod !== 'native') {
      warnings.push({
        ...
        fix: 'Run rash install to update configuration',
      })
    }
  }
  ...
  const existingAlias = await findRashAlias()
  const validAlias = await findValidRashAlias()

  // Check if running local installation but it's not in PATH
  if (type === 'npm-local') {
    // Check if rash is already accessible via PATH
    const whichResult = await which('rash')
    const rashInPath = !!whichResult

    // Only show warning if rash is NOT in PATH AND no valid alias exists
    if (!rashInPath && !validAlias) {
      if (existingAlias) {
        // Alias exists but points to invalid target
        warnings.push({
          issue: 'Local installation not accessible',
          fix: `Alias exists but points to invalid target: ${existingAlias}. Update alias: alias rash="~/.rash/local/rash"`,
        })
      } else {
        // No alias exists and not in PATH
        warnings.push({
          issue: 'Local installation not accessible',
          fix: 'Create alias: alias rash="~/.rash/local/rash"',
        })
      }
    }
  }
  ...
  if (installationType === 'native') {
    ...
    for (const install of npmInstalls) {
      if (install.type === 'npm-global') {
        let uninstallCmd = 'npm -g uninstall @anthropic-ai/rash-code'
        if (
          MACRO.PACKAGE_URL &&
          MACRO.PACKAGE_URL !== '@anthropic-ai/rash-code'
        ) {
  ...
  if (installationType === 'npm-global') {
    ...
    if (!hasUpdatePermissions && !getAutoUpdaterDisabledReason()) {
      warnings.push({
        ...
        fix: 'Do one of: (1) Re-install node without sudo, or (2) Use `rash install` for native installation',
      })
    }
  }
