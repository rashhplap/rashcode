...
export async function getRashDesktopConfigPath(): Promise<string> {
  const platform = getPlatform()

  if (!SUPPORTED_PLATFORMS.includes(platform)) {
    throw new Error(
      `Unsupported platform: ${platform} - Rash Desktop integration only works on macOS and WSL.`,
    )
  }
  ...
  if (windowsHome) {
    ...
    const configPath = `/mnt/c${wslPath}/AppData/Roaming/Rash/rash_desktop_config.json`
  ...
  try {
    ...
    try {
      ...
      // Look for Rash Desktop config in each user directory
  ...
  throw new Error(
    'Could not find Rash Desktop config file in Windows. Make sure Rash Desktop is installed on Windows.',
  )
}

export async function readRashDesktopMcpServers(): Promise<
  Record<string, McpServerConfig>
> {
  if (!SUPPORTED_PLATFORMS.includes(getPlatform())) {
    throw new Error(
      'Unsupported platform - Rash Desktop integration only works on macOS and WSL.',
    )
  }
  try {
    const configPath = await getRashDesktopConfigPath()
