 ...
 * socket (e.g., "rash-panel-a1b2c3d4") holds the shell session. Each Rash
 ...
 * Rash Code while the shell keeps running. Next toggle re-attaches to the
 * same session.
 *
 ...
 * Uses a unique socket per Rash Code instance (based on session ID)
...
export function getTerminalPanelSocket(): string {
  ...
  return `rash-panel-${sessionId.slice(0, 8)}`
}
...
class TerminalPanel {
  ...
  private createSession(): boolean {
    ...
    // Bind Meta+J (toggles back to Rash Code from inside the terminal)
    ...
    spawnSync('tmux', [
      ...
      'set-option', '-g', 'status-right', ' Alt+J to return to Rash ', ';',
