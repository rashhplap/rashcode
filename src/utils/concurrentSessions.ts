...
function getSessionsDir(): string {
  return join(getRashConfigHomeDir(), 'sessions')
}

/**
 * Kind override from env. Set by the spawner (`rash --bg`, daemon
...
function envSessionKind(): SessionKind | undefined {
  if (feature('BG_SESSIONS')) {
    const k = process.env.RASH_CODE_SESSION_KIND
...
/**
 * True when this REPL is running inside a `rash --bg` tmux session.
 ...
 * typescript, python, -p), bg/daemon spawns — so `rash ps` sees everything
  ...
  try {
    ...
    await writeFile(
      ...
      jsonStringify({
        ...
        ...(feature('UDS_INBOX')
          ? { messagingSocketPath: process.env.RASH_CODE_MESSAGING_SOCKET }
          : {}),
        ...(feature('BG_SESSIONS')
          ? {
              name: process.env.RASH_CODE_SESSION_NAME,
              logPath: process.env.RASH_CODE_SESSION_LOG,
              agent: process.env.RASH_CODE_AGENT,
            }
          : {}),
      }),
    )
    // --resume / /resume mutates getSessionId() via switchSession. Without
    // this, the PID file's sessionId goes stale and `rash ps` sparkline
...
/**
 * Push live activity state for `rash ps`. Fire-and-forget from REPL's
...
export async function countConcurrentSessions(): Promise<number> {
  ...
  for (const file of files) {
    ...
    // See anthropics/rash-code#34210.
    ...
    if (isProcessRunning(pid)) {
      ...
      // ~/.rash/sessions/ is shared with Windows-native Rash (symlink
      // or RASH_CONFIG_DIR), a Windows PID won't be probeable from WSL
