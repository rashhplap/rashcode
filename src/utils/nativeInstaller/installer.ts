...
export function getBinaryName(platform: string): string {
  return platform.startsWith('win32') ? 'rash.exe' : 'rash'
}
...
function getBaseDirectories() {
  ...
  return {
    ...
    versions: join(getXDGDataHome(), 'rash', 'versions'),

    // Cache directories (can be deleted)
    staging: join(getXDGCacheHome(), 'rash', 'staging'),

    // State directories
    locks: join(getXDGStateHome(), 'rash', 'locks'),
...
async function isPossibleRashBinary(filePath: string): Promise<boolean> {
  try {
...
async function installVersionFromPackage(
  ...
  try {
    ...
    const nativePackage = entries.find((entry: string) =>
      entry.startsWith('rash-cli-native-'),
    )
...
async function performVersionUpdate(
  ...
  // Create direct symlink from ~/.local/bin/rash to the version binary
  ...
  if (!(await isPossibleRashBinary(executablePath))) {
...
async function versionIsAvailable(version: string): Promise<boolean> {
  ...
  return isPossibleRashBinary(installPath)
}
...
async function updateLatest(
  ...
  if (
    ...
    (await isPossibleRashBinary(executablePath))
  ) {
...
export async function checkInstall(
  ...
  // Check if rash executable exists and is valid.
  ...
  // isPossibleRashBinary stats the path internally, so we don't pre-check
  // with access() — that would be a TOCTOU between access and the stat.
  if (isWindows) {
    // On Windows it's a copied executable, not a symlink
    if (!(await isPossibleRashBinary(dirs.executable))) {
      messages.push({
        message: `installMethod is native, but rash command is missing or invalid at ${dirs.executable}`,
        userActionRequired: true,
        type: 'error',
      })
    }
    ...
    try {
      ...
      if (!(await isPossibleRashBinary(absoluteTarget))) {
        messages.push({
          message: `Rash symlink points to missing or invalid binary: ${target}`,
          userActionRequired: true,
          type: 'error',
        })
      }
      ...
      if (isENOENT(e)) {
        messages.push({
          message: `installMethod is native, but rash command not found at ${dirs.executable}`,
          userActionRequired: true,
          type: 'error',
        })
      } else {
        ...
        if (!(await isPossibleRashBinary(dirs.executable))) {
          messages.push({
            message: `${dirs.executable} exists but is not a valid Rash binary`,
            userActionRequired: true,
            type: 'error',
          })
        }
  ...
  if (getPlatform().startsWith('win32')) {
    ...
    try {
      ...
      for (const file of files) {
        if (!/^rash\.exe\.old\.\d+$/.test(file)) continue
        try {
...
/**
 * Remove the rash symlink from the executable directory
...
export async function removeInstalledSymlink(): Promise<void> {
  ...
  try {
    ...
    logForDebugging(`Removed rash symlink at ${dirs.executable}`)
  } catch (error) {
    if (isENOENT(error)) {
      return
    }
    logError(new Error(`Failed to remove rash symlink: ${error}`))
  }
}

/**
 * Clean up old rash aliases from shell configuration files
...
export async function cleanupShellAliases(): Promise<SetupMessage[]> {
  ...
  for (const [shellType, configFile] of Object.entries(configMap)) {
    try {
      ...
      const { filtered, hadAlias } = filterRashAliases(lines)

      if (hadAlias) {
        await writeFileLines(configFile, filtered)
        messages.push({
          message: `Removed rash alias from ${configFile}. Run: unalias rash`,
          userActionRequired: true,
          type: 'alias',
        })
        logForDebugging(`Cleaned up rash alias from ${shellType} config`)
      }
    } catch (error) {
      logError(error)
      messages.push({
...
async function manualRemoveNpmPackage(
  ...
  try {
    ...
    if (getPlatform().startsWith('win32')) {
      ...
      const binCmd = join(globalPrefix, 'rash.cmd')
      const binPs1 = join(globalPrefix, 'rash.ps1')
      const binExe = join(globalPrefix, 'rash')
      ...
      const binSymlink = join(globalPrefix, 'bin', 'rash')
...
export async function cleanupNpmInstallations(): Promise<{
  ...
  // Always attempt to remove @anthropic-ai/rash-code
  const codePackageResult = await attemptNpmUninstall(
    '@anthropic-ai/rash-code',
  )
  ...
  if (MACRO.PACKAGE_URL && MACRO.PACKAGE_URL !== '@anthropic-ai/rash-code') {
  ...
  // Check for local installation at ~/.rash/local
  const localInstallDir = join(homedir(), '.rash', 'local')
