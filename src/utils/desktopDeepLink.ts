...
/**
 * Builds a deep link URL for Rash Desktop to resume a CLI session.
 * Format: rash://resume?session={sessionId}&cwd={cwd}
 * In dev mode: rash-dev://resume?session={sessionId}&cwd={cwd}
 */
function buildDesktopDeepLink(sessionId: string): string {
  const protocol = isDevMode() ? 'rash-dev' : 'rash'
...
/**
 * Check if Rash Desktop app is installed.
 * On macOS, checks for /Applications/Rash.app.
 * On Linux, checks if xdg-open can handle rash:// protocol.
...
async function isDesktopInstalled(): Promise<boolean> {
  ...
  if (platform === 'darwin') {
    // Check for Rash.app in /Applications
    return pathExists('/Applications/Rash.app')
  } else if (platform === 'linux') {
    // Check if xdg-mime can find a handler for rash://
...
/**
 * Detect the installed Rash Desktop version.
...
async function getDesktopVersion(): Promise<string | null> {
  ...
  if (platform === 'darwin') {
    const { code, stdout } = await execFileNoThrow('defaults', [
      'read',
      '/Applications/Rash.app/Contents/Info.plist',
    ...
    const installDir = join(localAppData, 'AnthropicRash')
    try {
...
/**
 * Build and open a deep link to resume the current session in Rash Desktop.
...
export async function openCurrentSessionInDesktop(): Promise<{
  ...
  if (!installed) {
    return {
      success: false,
      error:
        'Rash Desktop is not installed. Install it from https://rash.ai/download',
    }
  }
