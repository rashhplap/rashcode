...
import { accumulateUsage, updateUsage } from '../services/api/rash.js'
 ...
 * rash.ts (but only for older models that do not use adaptive thinking).
...
export type ForkedAgentParams = {
   ...
   * AND budget_tokens (via clamping in rash.ts). If the fork uses cacheSafeParams
...
export async function runForkedAgent({
  ...
  // tool_uses are repaired downstream by ensureToolResultPairing in rash.ts,
