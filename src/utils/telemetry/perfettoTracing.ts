/**
 * Perfetto Tracing for Rash Code (Ant-only)
 *
 ...
 *
 * Usage:
 * 1. Enable via RASH_CODE_PERFETTO_TRACE=1 or RASH_CODE_PERFETTO_TRACE=<path>
 * 2. Optionally set RASH_CODE_PERFETTO_WRITE_INTERVAL_S=<positive integer> to write the
 *    trace file periodically (default: write only on exit).
 * 3. Run Rash Code normally
 * 4. Trace file is written to ~/.rash/traces/trace-<session-id>.json
...
export function initializePerfettoTracing(): void {
  const envValue = process.env.RASH_CODE_PERFETTO_TRACE
  logForDebugging(
  ...
  if (feature('PERFETTO_TRACING')) {
    ...
    if (isEnvTruthy(envValue)) {
      const tracesDir = join(getRashConfigHomeDir(), 'traces')
    ...
    // Start periodic full-trace write if RASH_CODE_PERFETTO_WRITE_INTERVAL_S is a positive integer
    const intervalSec = parseInt(
      process.env.RASH_CODE_PERFETTO_WRITE_INTERVAL_S ?? '',
      10,
    )
    if (intervalSec > 0) {
