/**
 * Marketplace manager for Rash Code plugins
 *
...
export function getDeclaredMarketplaces(): Record<string, DeclaredMarketplace> {
  ...
  // An explicit extraKnownMarketplaces entry for rash-plugins-official
 ...
 *
 * Reads the configuration file at ~/.rash/plugins/known_marketplaces.json
 ...
 *     "installLocation": "/Users/me/.rash/plugins/marketplaces/official-marketplace.json",
 *     "lastUpdated": "2024-01-15T10:30:00.000Z"
 *   },
 *   "company-plugins": {
 *     "source": { "source": "github", "repo": "mycompany/plugins" },
 *     "installLocation": "/Users/me/.rash/plugins/marketplaces/company-plugins",
...
export async function loadKnownMarketplacesConfigSafe(): Promise<KnownMarketplacesConfig> {
  try {
    ...
    // corrupted user config isn't a Rash Code bug, shouldn't hit the error file.
    return {}
  }
}
 ...
 *
 * Writes the configuration to ~/.rash/plugins/known_marketplaces.json,
 ...
 *
 * Pulls latest changes with a configurable timeout (default 120s, override via RASH_CODE_PLUGIN_GIT_TIMEOUT_MS).
...
function getPluginGitTimeoutMs(): number {
  const envValue = process.env.RASH_CODE_PLUGIN_GIT_TIMEOUT_MS
  if (envValue) {
...
function enhanceGitPullErrorMessages(result: {
  ...
  if (result.error?.includes('timed out')) {
    ...
    return {
      ...result,
      stderr: `Git pull timed out after ${timeoutSec}s. Try increasing the timeout via RASH_CODE_PLUGIN_GIT_TIMEOUT_MS environment variable.\n\nOriginal error: ${result.stderr}`,
    }
  }
 ...
 *
 * Clones a git repository with a configurable timeout (default 120s, override via RASH_CODE_PLUGIN_GIT_TIMEOUT_MS)
...
export async function gitClone(
  ...
  if (result.error?.includes('timed out')) {
    return {
      ...result,
      stderr: `Git clone timed out after ${Math.round(timeoutMs / 1000)}s. The repository may be too large for the current timeout. Set RASH_CODE_PLUGIN_GIT_TIMEOUT_MS to increase it (e.g., 300000 for 5 minutes).\n\nOriginal error: ${result.stderr}`,
    }
  }
...
async function cacheMarketplaceFromUrl(
  ...
  const headers = {
    ...
    'User-Agent': 'Rash-Code-Plugin-Manager',
  }
 ...
 * - GitHub: Clones repo and looks for .rash-plugin/marketplace.json
    ...
    switch (source.source) {
      ...
      case 'github': {
        ...
        marketplacePath = join(
          temporaryCachePath,
          source.path || '.rash-plugin/marketplace.json',
        )
        break
      }
      ...
      case 'file': {
        ...
        // File sources point to .rash-plugin/marketplace.json, so the marketplace
        // root is two directories up (parent of .rash-plugin/)
      ...
      case 'directory': {
        // For directories, look for .rash-plugin/marketplace.json
        // Resolve to absolute so error messages show the actual path checked
        // (legacy known_marketplaces.json entries may have relative paths)
        const absPath = resolve(source.path)
        marketplacePath = join(absPath, '.rash-plugin', 'marketplace.json')
 ...
 * The configuration is saved to ~/.rash/plugins/known_marketplaces.json.
 *
  ...
  if (seedDir) {
    throw new Error(
      `Marketplace '${name}' is registered from the read-only seed directory ` +
        ...
        `To stop using its plugins: rash plugin disable <plugin>@${name}`,
    )
  }
...
async function readCachedMarketplace(
  ...
  // For git-sourced directories, the manifest lives at .rash-plugin/marketplace.json.
  ...
  const nestedPath = join(installLocation, '.rash-plugin', 'marketplace.json')
  try {
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `Rash Code version. Run 'rash marketplace remove ${name}' and ` +
...
export async function refreshAllMarketplaces(): Promise<void> {
  ...
  for (const [name, entry] of Object.entries(config)) {
    ...
    // hits this path on `rash plugin marketplace update` (no name arg).
  ...
  try {
      ...
      if (resolvedLoc !== cacheDir && !resolvedLoc.startsWith(cacheDir + sep)) {
        throw new Error(
          `Marketplace '${name}' has a corrupted installLocation ` +
            ...
            `Run: rash plugin marketplace remove "${name}" and re-add it.`,
        )
      }
    }
    ...
    if (source.source === 'github' || source.source === 'git') {
      ...
      if (source.source === 'github') {
        ...
        if (isEnvTruthy(process.env.RASH_CODE_REMOTE)) {
      ...
      try {
        ...
        const reason =
          name === 'rash-code-plugins'
            ? `We've deprecated "rash-code-plugins" in favor of "rash-plugins-official".`
        ...
        throw new Error(
          `The marketplace.json file is no longer present in this repository.\n\n` +
            ...
            `You can remove this marketplace with: rash plugin marketplace remove "${name}"`,
        )
      }
