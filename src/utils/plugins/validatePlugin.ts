 ...
 * one into the other. Surfaced as warnings by `rash plugin validate` since
...
function detectManifestType(
  ...
  // Check if it's in .rash-plugin directory
  if (dirName === '.rash-plugin') {
 ...
 * '..' a user added to "climb out of .rash-plugin/" is unnecessary. Callers pass `hint`
...
// they expect paths to resolve relative to marketplace.json (inside .rash-plugin/),
...
function marketplaceSourceHint(p: string): string {
  ...
  // .rash-plugin/" is unnecessary since paths already start at the repo root.
  ...
  return (
    ...
    'containing .rash-plugin/), not relative to marketplace.json. ' +
...
export async function validatePluginManifest(
  ...
  // them. `rash plugin validate` is a developer tool — authors running it
      ...
      for (const key of strayKeys) {
        ...
        warnings.push({
          ...
          message:
            ...
            `not plugin.json. It's harmless here but unused — Rash Code ` +
  ...
  if (result.success) {
    ...
    // but the Rash.ai marketplace sync rejects non-kebab names. Surfacing
    // this here lets authors catch it in CI before the sync fails on them.
    if (!/^[a-z0-9]+(-[a-z0-9]+)*$/.test(manifest.name)) {
      warnings.push({
        path: 'name',
        message:
          `Plugin name "${manifest.name}" is not kebab-case. Rash Code accepts ` +
          `it, but the Rash.ai marketplace sync requires kebab-case ` +
  ...
  if (result.success) {
    ...
    if (marketplace.plugins) {
      ...
      const marketplaceRoot =
        path.basename(manifestDir) === '.rash-plugin'
      ...
      for (const [i, entry] of marketplace.plugins.entries()) {
        ...
        if (manifestVersion && manifestVersion !== entry.version) {
          warnings.push({
            ...
            message:
              `Entry declares version "${entry.version}" but ${entry.source}/.rash-plugin/plugin.json says "${manifestVersion}". ` +
 ...
 * for the load path, but authors running `rash plugin validate` want a hard
...
function validateComponentFile(
  ...
  if (fm.description !== undefined) {
    ...
    warnings.push({
      path: 'description',
      message:
        `No description in frontmatter. A description helps users and Rash ` +
...
export async function validateManifest(
  ...
  if (stats?.isDirectory()) {
    // Look for manifest files in .rash-plugin directory
    ...
    const pluginPath = path.join(absolutePath, '.rash-plugin', 'plugin.json')
    ...
    return {
      ...
      errors: [
        {
          path: 'directory',
          message: `No manifest found in directory. Expected .rash-plugin/marketplace.json or .rash-plugin/plugin.json`,
        },
      ],
      warnings: [],
      filePath: absolutePath,
