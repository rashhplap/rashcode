...
export function savePluginOptions(
  ...
  // pluginConfigs into ~/.rash/settings.json. Same pattern exists in
...
export function deletePluginOptions(pluginId: string): void {
  ...
  // removal. Cast is deliberate (RASH.md's 10% case): adding z.undefined()
...
/**
 * Substitute ${RASH_PLUGIN_ROOT} and ${RASH_PLUGIN_DATA} with their paths.
 * On Windows, normalizes backslashes to forward slashes so shell commands
 * don't interpret them as escape characters.
 *
 * ${RASH_PLUGIN_ROOT} — version-scoped install dir (recreated on update)
 * ${RASH_PLUGIN_DATA} — persistent state dir (survives updates)
 *
...
export function substitutePluginVariables(
  ...
  let out = value.replace(/\$\{RASH_PLUGIN_ROOT\}/g, () =>
    normalize(plugin.path),
  )
  // source can be absent (e.g. hooks where pluginRoot is a skill root without
  // a plugin context). In that case ${RASH_PLUGIN_DATA} is left literal.
  if (plugin.source) {
    const source = plugin.source
    out = out.replace(/\$\{RASH_PLUGIN_DATA\}/g, () =>
