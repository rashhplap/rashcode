 ...
 * Handles ${RASH_PLUGIN_ROOT}, ${user_config.X}, and general ${VAR} substitution
...
export function resolvePluginMcpEnvironment(
  ...
  switch (config.type) {
    ...
    case 'stdio': {
      ...
      // Resolve environment variables and add RASH_PLUGIN_ROOT / RASH_PLUGIN_DATA
      const resolvedEnv: Record<string, string> = {
        RASH_PLUGIN_ROOT: plugin.path,
        RASH_PLUGIN_DATA: getPluginDataDir(plugin.source),
        ...(stdioConfig.env || {}),
      }
      for (const [key, value] of Object.entries(resolvedEnv)) {
        if (key !== 'RASH_PLUGIN_ROOT' && key !== 'RASH_PLUGIN_DATA') {
    ...
    // For other types (sse-ide, ws-ide, sdk, rashai-proxy), pass through unchanged
    case 'sse-ide':
    case 'ws-ide':
    case 'sdk':
    case 'rashai-proxy':
