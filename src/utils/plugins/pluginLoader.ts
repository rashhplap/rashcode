 ...
 *
 * This module is responsible for discovering, loading, and validating Rash Code plugins
 ...
 * Format: ~/.rash/plugins/cache/{marketplace}/{plugin}/{version}/
 *
 * @param pluginId - Plugin identifier in format "name@marketplace"
 * @param version - Version string (semver, git SHA, etc.)
 ...
 * Format: ~/.rash/plugins/cache/{plugin-name}/
 *
...
async function installFromGitHub(
  ...
  const gitUrl = isEnvTruthy(process.env.RASH_CODE_REMOTE)
 ...
 * RASH_CODE_REMOTE) or any URL that passes validateGitUrl (https, http,
 * file, git@ ssh).
 */
...
export async function cachePlugin(
  ...
  const manifestPath = join(tempPath, '.rash-plugin', 'plugin.json')
 ...
 * @param source - Source description for default manifest (e.g., "git:repo" or ".rash-plugin/name")
 ...
 * @param source - Source identifier (e.g., "git:repo", ".rash-plugin/my-plugin")
...
export async function createPluginFromPath(
  ...
  const manifestPath = join(pluginPath, '.rash-plugin', 'plugin.json')
  ...
  const plugin: LoadedPlugin = {
    ...
    source, // Source identifier (e.g., "git:repo" or ".rash-plugin/name")
 ...
 *
 * RASH_CODE_SYNC_PLUGIN_INSTALL=1 delegates to the full loader — that
 * mode explicitly opts into blocking install before first query, and
 * main.tsx's getRashCodeMcpConfigs()/getInitialSettings().agent run
 ...
 * - After modifying .rash-plugin/ directory (for export)
