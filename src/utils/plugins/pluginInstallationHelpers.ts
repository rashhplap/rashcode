 ...
 * 1. Caching a plugin to ~/.rash/plugins/cache/
  ...
  if (cacheResult.path !== versionedPath) {
    ...
    if (isSubdirectory) {
      ...
      const tempPath = join(
        ...
        `.rash-plugin-temp-${Date.now()}-${randomBytes(4).toString('hex')}`,
      )
...
export async function installPluginFromMarketplace({
  ...
  try {
    ...
    // non-official) because dbt external_rash_code_plugin_installs.sql
