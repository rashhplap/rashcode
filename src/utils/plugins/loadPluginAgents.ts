...
async function loadAgentFromFile(
  ...
  try {
    ...
    // Substitute ${RASH_PLUGIN_ROOT} so agents can reference bundled files,
    ...
    // time. For this level of control, define the agent in .rash/agents/
    ...
    for (const field of ['permissionMode', 'hooks', 'mcpServers'] as const) {
      if (frontmatter[field] !== undefined) {
        logForDebugging(
          `Plugin agent file ${filePath} sets ${field}, which is ignored for plugin agents. Use .rash/agents/ for this level of control.`,
          { level: 'warn' },
        )
      }
    }
