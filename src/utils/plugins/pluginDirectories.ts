 ...
 * - CLI flag: --cowork
 * - Environment variable: RASH_CODE_USE_COWORK_PLUGINS
 *
 * The base directory can be overridden via RASH_CODE_PLUGIN_CACHE_DIR.
 */
 ...
 * 2. Environment variable RASH_CODE_USE_COWORK_PLUGINS
 * 3. Default: 'plugins'
 */
function getPluginsDirectoryName(): string {
  ...
  if (isEnvTruthy(process.env.RASH_CODE_USE_COWORK_PLUGINS)) {
    return COWORK_PLUGINS_DIR
  }
  return PLUGINS_DIR
}
 ...
 *
 * Priority:
 * 1. RASH_CODE_PLUGIN_CACHE_DIR env var (explicit override)
 * 2. Default: ~/.rash/plugins or ~/.rash/cowork_plugins
 */
export function getPluginsDirectory(): string {
  // expandTilde: when RASH_CODE_PLUGIN_CACHE_DIR is set via settings.json
  // `env` (not shell), ~ is not expanded by the shell. Without this, a value
  // like "~/.rash/plugins" becomes a literal `~` directory created in the
  // cwd of every project (gh-30794 / CC-212).
  const envOverride = process.env.RASH_CODE_PLUGIN_CACHE_DIR
  if (envOverride) {
    return expandTilde(envOverride)
  }
  return join(getRashConfigHomeDir(), getPluginsDirectoryName())
}
 ...
 * image and point RASH_CODE_PLUGIN_SEED_DIR at it. CC will use it as a
...
export function getPluginSeedDirs(): string[] {
  ...
  const raw = process.env.RASH_CODE_PLUGIN_SEED_DIR
  if (!raw) return []
 ...
 * ${RASH_PLUGIN_DATA}. Unlike the version-scoped install cache
 * (${RASH_PLUGIN_ROOT}, which is orphaned and GC'd on every update),
 ...
 * .replace() so this isn't invoked unless ${RASH_PLUGIN_DATA} is present
