 ...
 * When RASH_CODE_PLUGIN_USE_ZIP_CACHE is enabled and RASH_CODE_PLUGIN_CACHE_DIR
...
export function isPluginZipCacheEnabled(): boolean {
  return isEnvTruthy(process.env.RASH_CODE_PLUGIN_USE_ZIP_CACHE)
}

/**
 * Get the path to the zip cache directory.
 * Requires RASH_CODE_PLUGIN_CACHE_DIR to be set.
...
export function getPluginZipCachePath(): string | undefined {
  ...
  const dir = process.env.RASH_CODE_PLUGIN_CACHE_DIR
...
export async function getSessionPluginCachePath(): Promise<string> {
  ...
  if (!sessionPluginCachePromise) {
    sessionPluginCachePromise = (async () => {
      ...
      const dir = join(tmpdir(), `rash-plugin-session-${suffix}`)
...
async function collectFilesForZip(
  ...
  // See also: markdownConfigLoader.ts getFileIdentity, anthropics/rash-code#13893
  try {
