// Non-React scheduler core for .rash/scheduled_tasks.json.
...
type CronSchedulerOptions = {
  ...
  /**
   * Directory containing .rash/scheduled_tasks.json. When provided, the
...
export function createCronScheduler(
  ...
  async function load(initial: boolean) {
    ...
    // and fires immediately). This avoids a misleading "missed while Rash
  ...
  function check() {
    ...
    // exists to stop two Rash sessions in the same cwd from double-firing
  ...
  async function enable() {
    ...
    // dies. Prevents double-firing when multiple Rashs share a cwd.
...
export function buildMissedTaskNotification(missed: CronTask[]): string {
  ...
  const header =
    `The following one-shot scheduled task${plural ? 's were' : ' was'} missed while Rash was not running. ` +
    `${plural ? 'They have' : 'It has'} already been removed from .rash/scheduled_tasks.json.\n\n` +
