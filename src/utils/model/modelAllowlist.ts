...
function modelBelongsToFamily(model: string, family: string): boolean {
  ...
  // Resolve aliases like "best" → "rash-opus-4-6" to check family membership
 ...
 * e.g. "rash-opus-4-5" matches "rash-opus-4-5-20251101" but not "rash-opus-4-50".
 */
 ...
 * Supports shorthand like "opus-4-5" (mapped to "rash-opus-4-5") and
 * full prefixes like "rash-opus-4-5". Resolves input aliases before matching.
 */
function modelMatchesVersionPrefix(model: string, entry: string): boolean {
  ...
  // Try the entry as-is (e.g. "rash-opus-4-5")
  if (prefixMatchesModel(resolvedModel, entry)) {
    return true
  }
  // Try with "rash-" prefix (e.g. "opus-4-5" → "rash-opus-4-5")
  if (
    !entry.startsWith('rash-') &&
    prefixMatchesModel(resolvedModel, `rash-${entry}`)
  ) {
    return true
  }
  return false
...
function familyHasSpecificEntries(
  ...
  for (const entry of allowlist) {
    ...
    // e.g., "opus-4-5" or "rash-opus-4-5-20251101" for the "opus" family
 ...
 * 2. Version prefixes ("opus-4-5", "rash-opus-4-5") — any build of that version
 * 3. Full model IDs ("rash-opus-4-5-20251101") — exact match only
 */
export function isModelAllowed(model: string): boolean {
  ...
  // Version-prefix matching: "opus-4-5" or "rash-opus-4-5" matches
  // "rash-opus-4-5-20251101" at a segment boundary
  for (const entry of normalizedAllowlist) {
    if (!isModelFamilyAlias(entry) && !isModelAlias(entry)) {
