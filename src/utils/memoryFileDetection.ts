...
/**
 * Detects if a file path is a session-related file under ~/.rash.
...
/**
 * Check if a file is a Rash-managed memory file (NOT user-managed instruction files).
 * Includes: auto-memory (memdir), agent memory, session memory/transcripts.
 * Excludes: RASH.md, RASH.local.md, .rash/rules/*.md (user-managed).
 *
...
export function isMemoryDirectory(dirPath: string): boolean {
  ...
  // Check the auto-memory path override (RASH_COWORK_MEMORY_PATH_OVERRIDE)
  if (isAutoMemoryEnabled()) {
  ...
  const configDirCmp = toComparable(getRashConfigHomeDir())
...
// Excludes RASH.md, RASH.local.md, .rash/rules/ (user-managed).
