...
import { clearMemoryFileCaches } from './utils/rashmd.js'
...
export async function setup(
  ...
  if (!nodeVersion || parseInt(nodeVersion) < 18) {
    ...
    console.error(
      chalk.bold.red(
        'Error: Rash Code requires Node.js version 18 or higher.',
      ),
    )
    process.exit(1)
  }
  ...
  if (!isBareMode() || messagingSocketPath !== undefined) {
    ...
    // and $RASH_CODE_MESSAGING_SOCKET is exported before any hook
  ...
  if (worktreeEnabled) {
    ...
    // .rash/settings.json. Re-read from the worktree and re-capture hooks.
    updateHooksConfigSnapshot()
  }
  ...
  // When RASH_CODE_SYNC_PLUGIN_INSTALL is set, skip all plugin prefetch.
  ...
  const skipPluginPrefetch =
    (getIsNonInteractiveSession() &&
      isEnvTruthy(process.env.RASH_CODE_SYNC_PLUGIN_INSTALL)) ||
  ...
  if (
    ...
    if (
      ...
      !isEnvTruthy(process.env.RASH_CODE_BUBBLEWRAP)
    ) {
      // biome-ignore lint/suspicious/noConsole:: intentional console output
      console.error(
    ...
    if (
      ...
      process.env.RASH_CODE_ENTRYPOINT !== 'local-agent' &&
      // Same for CCD (Rash Code in Desktop) — apps#29127 passes the flag
