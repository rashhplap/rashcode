# rashcode Local Agent Playbook

This playbook is a practical guide to run rashcode with a local model (Ollama), work safely, and get strong day-to-day results.
...
If everything is healthy, rashcode starts directly.
...
Or let rashcode recommend the best local model for your goal:
...
```powershell
Get-Content .\.rashcode-profile.json
```
...
```powershell
cd C:\Users\Lucas Pedry\Documents\rashcode\rashcode
bun run dev:profile
```
...
- Keep `.rashcode-profile.json` local (already gitignored).
